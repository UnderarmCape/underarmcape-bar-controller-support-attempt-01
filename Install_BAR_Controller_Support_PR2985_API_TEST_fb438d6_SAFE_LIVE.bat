...
if /I not "%~1"=="__RUN__" (
    ...
    set "BOOT_LOG=%USERPROFILE%\Desktop\BAR_Controller_Install_PR2985_API_TEST_fb438d6_SAFE_LIVE_!BOOT_STAMP!.log"

    echo ============================================================
    echo BAR Controller Support PR #2985 API Test - fb438d6 SAFE LIVE Installer
    echo ============================================================
    echo.
...
set "INSTALL_LOG=%~2"
title BAR Controller Support PR #2985 API Test - fb438d6 SAFE LIVE Installer
...
echo It uses the reviewed Recoil PR #2985 API engine build fb438d6 and the matching BAR controller widget.
...
set "BAR_RELEASE_TAG=controller-support-pr2985-api-test-fb438d6"
set "CHOBBY_REPO=https://github.com/beyond-all-reason/BYAR-Chobby.git"

set "ENGINE_ASSET_NAME=recoil-controller-amd64-windows-fb438d6e2fd1d41e96048ee87ca652c8c4e45cf6.zip"
set "ENGINE_URL=https://github.com/UnderarmCape/controllersupport-RecoilEngine-attempt-01/releases/download/controller-engine-pr2985-review-api-test-fb438d6/%ENGINE_ASSET_NAME%"
...
set "WORK=%TEMP%\BAR-Controller-Support-PR2985-API-TEST-fb438d6-SAFE-%STAMP%"
...
if exist "%LOCAL_WIDGET%" (
    ...
    echo Downloading matching widget from BAR PR2985 API test release...
    echo.
    echo URL:
    echo "%WIDGET_URL%"
    echo.
...
echo.
echo Healthy signs:
echo   Spring Engine Version: fb438d6 build/pr-controller-input-lua-api-artifact
echo   Controller connected: Xbox ...
echo   No repeating ButtonDown/ButtonUp/AxisMotion spam during normal controller use
echo   Widget Raw state reports state table ok and live axes/buttons update
echo.
