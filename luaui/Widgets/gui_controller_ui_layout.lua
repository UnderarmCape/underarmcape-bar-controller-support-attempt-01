...
local DEFAULTS = {
	...
	authoring = {
		...
		shortcutSearch = "Ctrl+F", shortcutClose = "Escape", developerAuthoring = false,
	},
	components = {
		hints = {
			...
			showTapHold = true, maxItems = 14, expanded = false,
		},
		bindingsButton = {
		...
		hotSlots = { enabled = true, x = 0.5, y = 0.08, scale = 1, opacity = 1, fontScale = 1,
			iconScale = 1, slotSize = 42, slotGap = 5, slotCount = 10, showLabel = true,
			showCounts = true, showAuto = true, backgroundOpacity = 0.72, anchor = "bottomcenter" },
		...
		editorLauncher = { enabled = true, x = 0.79, y = 1044 / 1080, scale = 1, opacity = 0.9,
			fontScale = 1, width = 116 / 1920, height = 28 / 1080, backgroundOpacity = 0.88, anchor = "bottomleft" },
...
local authorData = { favorites = {}, savedValues = {}, componentPresets = {}, recent = {} }
local history = { undo = {}, redo = {}, active = nil, dirty = false, lastSaved = deepCopy(DEFAULTS), suppress = false }
...
local editor = {
	...
	undo = nil, redo = nil, save = nil, context = nil, draft = nil, reload = nil,
}

local tabs = { "General", "Hints", "Hot Slots", "Launchers", "Radials", "Status", "Other", "Theme", "Authoring", "Recovery" }
...
addProperty("Hints", "hints", "overflow", "Long-label strategy", "enum", nil, nil, nil, "Basic", { "Wrap", "Shrink", "Truncate" })
...
for _, row in ipairs({
	...
	{ "slotSize", "Slot size", "number", 24, 84, 2 }, { "slotGap", "Slot gap", "number", 0, 24, 1 },
...
}) do addProperty("Hot Slots", "hotSlots", row[1], row[2], row[3], row[4], row[5], row[6], row[7]) end
...
addProperty("Theme", "theme", "preset", "Theme preset", "enum", nil, nil, nil, "Basic", { "Ocean", "High Contrast", "Soft", "Colorblind" })
...
local function beginMutation(label)
	...
	pushBounded(history.undo, { settings = deepCopy(settings), label = label })
	history.redo = {}
...
local function getScope(scope)
	if scope == "global" or scope == "theme" or scope == "authoring" then return settings[scope], DEFAULTS[scope] end
...
local function resetComponent(scope)
	...
	if scope == "global" or scope == "theme" or scope == "authoring" then settings[scope] = deepCopy(DEFAULTS[scope])
...
local function undo()
	...
	pushBounded(history.redo, { settings = deepCopy(settings), label = entry.label })
	settings = deepCopy(entry.settings); history.dirty = not deepEqual(settings, history.lastSaved)
...
local function redo()
	...
	pushBounded(history.undo, { settings = deepCopy(settings), label = entry.label })
	settings = deepCopy(entry.settings); history.dirty = not deepEqual(settings, history.lastSaved)
...
local function rebuildHints(context, api)
	...
	for _, def in pairs(hintRegistry) do
		...
		if mode == "Everything" and def.group ~= "developer" then available = true end
...
local function saveCurrentValue(row)
	if not row then return end
	local target = getScope(row[1]); authorData.savedValues[rowID(row)] = deepCopy(target[row[2]])
	editor.status = "Saved value for " .. rowID(row)
end

local function applySavedValue(row)
	if not row then return end
	local value = authorData.savedValues[rowID(row)]
	if value == nil then editor.status = "No saved value for " .. rowID(row); return end
	setValue(row[1], row[2], deepCopy(value)); editor.status = "Applied saved value for " .. rowID(row)
end

local function saveComponentPreset(component)
	local target = settings.components[component]
	if not target then return end
	authorData.componentPresets[component] = deepCopy(target); editor.status = "Saved component preset: " .. component
end

local function applyComponentPreset(component)
	local preset = authorData.componentPresets[component]
	if not preset or not settings.components[component] then editor.status = "No preset saved for " .. tostring(component); return end
	local ownMutation = beginMutation("Apply preset " .. component)
	mergeValidated(settings.components[component], preset, DEFAULTS.components[component])
	history.dirty = true; touch(); if ownMutation then endMutation() end
	editor.status = "Applied component preset: " .. component
end
...
local function drawHints()
	...
	if component.overflow == "Wrap" then rowHeight = rowHeight + fontSize * 0.72 end
	local displayCount = #visibleHints
	if not component.expanded then displayCount = math.min(displayCount, math.floor(component.maxItems or 14)) end
	...
	local panelHeight = component.padding * 2 * scale + rows * rowHeight + moreHeight
	local margin = settings.global.safeMargin * automaticScale
	...
	drawOutline(x1, y1, x1 + panelWidth, y1 + panelHeight, { theme.accentR, theme.accentG, theme.accentB, component.borderOpacity * opacity })
	...
	for index = 1, displayCount do
		...
		local ry = y1 + panelHeight - component.padding * scale - moreHeight - (row + 1) * rowHeight
		...
		local labelX = rx + chipWidth + component.iconTextSpacing * scale
		local availableWidth = math.max(24, rx + columnWidth - labelX)
		local displayLabel, secondLine, displaySize = label, nil, fontSize
		if component.overflow == "Shrink" then
			while displaySize > fontSize * 0.62 and textWidth(displayLabel, displaySize) > availableWidth do displaySize = displaySize - 1 end
		...
		else displayLabel, secondLine = wrapText(displayLabel, displaySize, availableWidth) end
		glColor(theme.foregroundR, theme.foregroundG, theme.foregroundB, component.textOpacity * opacity)
		local labelY = ry + (rowHeight - displaySize) * 0.5 + (secondLine and displaySize * 0.28 or 0)
		glText(displayLabel, labelX, labelY, displaySize, "o")
		if secondLine then glText(secondLine, labelX, labelY - displaySize * 0.82, displaySize * 0.82, "o") end
...
local previewContexts = { "Live", "Normal", "Build Radial", "Factory Radial", "Placement", "Tactical", "Selection", "Control Groups", "Pregame", "Mouse" }

local function canvasBounds(name, baseWidth, baseHeight)
	if name == "hotSlots" then
		local item = settings.components.hotSlots
		baseWidth = (item.slotCount or 10) * (item.slotSize or 42) + math.max(0, (item.slotCount or 10) - 1) * (item.slotGap or 5) + 24
		baseHeight = (item.slotSize or 42) + (item.showLabel and 34 or 16)
...
local function moveComponent(name, x1, y1, bounds)
	...
	local snappedX, guideX = snapPixel(x1, { margin, viewX * 0.5 - bounds.width * 0.5, viewX - margin - bounds.width })
	local snappedY, guideY = snapPixel(y1, { margin, viewY * 0.5 - bounds.height * 0.5, viewY - margin - bounds.height })
...
local function previewContext(base)
	...
	if mode == "Normal" then result.hasSelection, result.selectedCount, result.hasBuilder = true, 1, true
	elseif mode == "Build Radial" then result.buildMenuOpen, result.hasBuilder, result.hasSelection = true, true, true
	...
	elseif mode == "Placement" then result.buildPlacement, result.hasBuilder, result.hasSelection = true, true, true
	elseif mode == "Tactical" then result.tacticalRadialOpen, result.hasSelection = true, true
	elseif mode == "Selection" then result.selectionRadialOpen, result.areaSelection = true, true
	elseif mode == "Control Groups" then result.controlGroupLayer, result.hasSelection = true, true
	elseif mode == "Pregame" then result.pregame = true
	elseif mode == "Mouse" then result.mouseMode = true end
	result.layoutEditorOpen = false
	return result
end
...
local function drawEditor()
	...
	editor.draft = settings.authoring.developerAuthoring and toolButton(tx, 84, "Draft") or nil
	...
	for visibleIndex = 1, maxVisible do
		...
		editor.hits[#editor.hits + 1] = { x1 = b.x1 + 16, y1 = y1, x2 = b.x2 - 16, y2 = y2, index = index,
			minus = minus, plus = plus, star = star, saveValue = saveValueHit, applyValue = applyValueHit, row = row }
	end
	...
	editor.alignHits = {}
	for index, item in ipairs({ { "left", "L" }, { "hcenter", "HC" }, { "right", "R" }, { "bottom", "B" }, { "vcenter", "VC" }, { "top", "T" } }) do
...
local function getThemeColor(role, alpha)
	local theme = settings.theme
...
local function writeAuthoringDraft()
	...
	local document = { kind = "bar-controller-ui-defaults", schemaVersion = SCHEMA_VERSION,
		...
		settings = settings, enforcedSettings = {}, enforcedPaths = {} }
...
function widget:Initialize()
	...
	if not history.dirty then history.lastSaved = deepCopy(settings) end
	WG.ControllerUISettings = {
		...
		GetLayerStatus = function() return { activeVersion = shipping.activeVersion, status = shipping.status,
			dirty = history.dirty, undo = #history.undo, redo = #history.redo } end,
...
function widget:MousePress(x, y, button)
	...
	if insidePanel then
		...
		for _, hit in ipairs(editor.alignHits or {}) do if button == 1 and pointInside(hit, x, y) then alignSelected(hit.mode); return true end end
...
function widget:MouseRelease()
	local handled = editor.dragComponent or editor.dragging or editor.resizing or editor.held
	editor.dragComponent, editor.dragging, editor.resizing, editor.held = nil, false, false, nil
...
function widget:KeyPress(key, mods, isRepeat, label)
	...
	if KEYSYMS and key == KEYSYMS.ESCAPE then setEditorOpen(false); return true end
	...
	if KEYSYMS and key == KEYSYMS.LEFT then adjustRow(-1, mods and mods.shift and 10 or 1); return true end
	if KEYSYMS and key == KEYSYMS.RIGHT then adjustRow(1, mods and mods.shift and 10 or 1); return true end
	if KEYSYMS and key == KEYSYMS.TAB then editor.row = (editor.row % math.max(1, #currentRows())) + 1; return true end
	if KEYSYMS and key == KEYSYMS.DELETE then local row = currentRow(); if row then setValue(row[1], row[2], getScope(row[1]) and select(2, getScope(row[1]))[row[2]]) end; return true end
	return true
end
...
function widget:TextCommand(command)
	...
	local shortcutKeys = { undo = "shortcutUndo", redo = "shortcutRedo", save = "shortcutSave", search = "shortcutSearch" }
...
function widget:GetConfigData()
	return { schemaVersion = SCHEMA_VERSION, personalSettings = personalSettings or history.lastSaved,
		settings = personalSettings or history.lastSaved, authorData = authorData,
		recovery = history.dirty and { dirty = true, settings = settings } or { dirty = false },
...
function widget:SetConfigData(data)
	...
	if savedSchema ~= nil and savedSchema ~= 1 and savedSchema ~= 2 and savedSchema ~= SCHEMA_VERSION then
		personalSettings, authorData = nil, { favorites = {}, savedValues = {}, componentPresets = {}, recent = {} }
		loadShippingDocuments(); resolveSettings(nil); history.lastSaved = deepCopy(settings); return
	end
	...
	if type(data.authorData) == "table" then
		authorData.favorites = type(data.authorData.favorites) == "table" and deepCopy(data.authorData.favorites) or {}
		authorData.savedValues = type(data.authorData.savedValues) == "table" and deepCopy(data.authorData.savedValues) or {}
		authorData.componentPresets = type(data.authorData.componentPresets) == "table" and deepCopy(data.authorData.componentPresets) or {}
		authorData.recent = type(data.authorData.recent) == "table" and deepCopy(data.authorData.recent) or {}
	end
	loadShippingDocuments(); resolveSettings(nil); history.lastSaved = deepCopy(settings)
	if type(data.recovery) == "table" and data.recovery.dirty == true and type(data.recovery.settings) == "table" then
		resolveSettings(data.recovery.settings); history.dirty = true; editor.recoveryAvailable = true; editor.status = "Recovered unsaved authoring changes"
