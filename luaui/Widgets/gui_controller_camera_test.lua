...
ControllerNativeBuildCellRenderer = ControllerNativeBuildCellRenderer or nil
do
	local ok, module = pcall(VFS.Include, "LuaUI/Include/controller_native_build_cell_renderer.lua")
	if ok and type(module) == "table" then ControllerNativeBuildCellRenderer = module end
end
...
local BuildRadialPageColors = {
	...
	combat = {
		...
		accent = { 1.0, 0.42, 0.24, 1.0 },
		label = { 1.0, 0.72, 0.54, 1.0 },
	},
	utility = {
		fill = { 0.12, 0.08, 0.45, 0.45 },
		accent = { 0.76, 0.64, 1.0, 1.0 },
		label = { 0.88, 0.82, 1.0, 1.0 },
	},
	build = {
		fill = { 0.45, 0.28, 0.04, 0.45 },
		accent = { 1.0, 0.75, 0.1, 1.0 },
	},
	constructors = { fill = { 0.36, 0.22, 0.04, 0.45 }, accent = { 1.0, 0.72, 0.12, 1.0 } },
...
ControllerCameraTestBuildMenu = ControllerCameraTestBuildMenu or {
	...
	radialPageTraversal = {},
	radialTraversalIndex = 1,
	radialTraversalCount = 1,
	...
	factoryQuotaModeActive = false,
	factoryQuotaModeSource = "unknown",
	factoryQueueRefreshReason = "initial",
...
ControllerCameraTestBuildPlacement = ControllerCameraTestBuildPlacement or {
	...
	distributedGridArmed = false,
	distributedGridChordOwned = false,
	distributedGridResult = "none",
}
...
ControllerCameraTestDisassemble = ControllerCameraTestDisassemble or {
	...
	areaReclaim = { active = false, anchorUnitID = nil, unitDefID = nil, x = nil, y = nil, z = nil, radius = 24, candidates = {} },
	lbA = { pressActive = false, startedAt = 0, targetID = nil, unitDefID = nil, targetInfo = nil, holdFired = false },
	pendingX = { pressActive = false, startedAt = 0, holdFired = false, targetInfo = nil, groundInfo = nil },
	doubleB = ControllerDisassembleBehavior and ControllerDisassembleBehavior.NewDoubleBTap()
		or { lastPressAt = -10, releaseArmed = true, lastEvent = "idle" },
}
...
ControllerCameraTestGeneralInsert = ControllerCameraTestGeneralInsert
	or { active = false, waitingForRBRelease = false, lastResult = "idle", lastIssued = false, lastSource = "none" }
ControllerCameraTestUnitTypeNavigation = ControllerCameraTestUnitTypeNavigation or {
	...
	representativeRule = "lowest-unit-id",
}
...
ControllerCameraTestCommandCapabilities = ControllerCameraTestCommandCapabilities or {
	selectionRevision = -1,
	activeSignature = "",
	values = {},
}
...
ControllerCameraTestNativeTargeting.trace = ControllerCameraTestNativeTargeting.trace or {}
ControllerCameraTestNativeTargeting.traceRevision = ControllerCameraTestNativeTargeting.traceRevision or 0
...
function ControllerCameraTestGetDefaultSettings()
	return {
		...
		selectionTransitionDebug = false,
		...
		nativePanelsDebugVisible = false,
	}
end
...
function ControllerCameraTestClampSetting(name, value)
	...
	local ranges = {
		...
		disassembleAreaReclaimRadius = { 12, 1200 },
		disassembleToggleHoldSeconds = { 0.15, 1.50 },
		disassembleUnusedTimeoutSeconds = { 30.0, 30.0 },
		lbTapMaxSeconds = { 0.08, 0.50 },
		lbTacticalHoldSeconds = { 0.08, 0.50 },
		rbTapMaxSeconds = { 0.08, 0.50 },
		rbChordGraceSeconds = { 0.05, 0.40 },
...
function ControllerCameraTestApplySettingsDefaults()
	...
	settings.disassembleAreaReclaimRadius = ControllerCameraTestClampSetting("disassembleAreaReclaimRadius",
		settings.disassembleAreaReclaimRadius or defaults.disassembleAreaReclaimRadius)
	...
	settings.disassembleUnusedTimeoutSeconds = 30.0
	...
	settings.rbTapMaxSeconds = ControllerCameraTestClampSetting("rbTapMaxSeconds", settings.rbTapMaxSeconds or defaults.rbTapMaxSeconds)
	settings.rbChordGraceSeconds = ControllerCameraTestClampSetting("rbChordGraceSeconds", settings.rbChordGraceSeconds or defaults.rbChordGraceSeconds)
	...
	settings.nativePanelsDebugVisible = settings.nativePanelsDebugVisible == true
...
local function attemptReticleSelection(forcedUnitID)
	...
	local unitID = tonumber(forcedUnitID)
	if not unitID then
...
function ControllerCameraTestTraceSingleSelection(event, details)
	if not ControllerCameraTestSettings.selectionTransitionDebug then return end
...
function ControllerCameraTestGetSettingsDefinitions()
	...
	local ranges = {
		...
		rbTapMaxSeconds = { 0.08, 0.50, 0.01, "number", 2 },
		rbChordGraceSeconds = { 0.05, 0.40, 0.01, "number", 2 },
		disassembleAreaReclaimRadius = { 12, 1200, 4, "number", 0 },
		...
		nativePanelsDebugVisible = { 0, 1, 1, "boolean", 0 },
		...
		selectionTransitionDebug = { 0, 1, 1, "boolean", 0 },
...
function ControllerCameraTestResetHybridRadials(reason)
	...
	placement.distributedGridArmed, placement.distributedGridChordOwned = false, false
	placement.distributedGridResult = "reset"
	...
	if ControllerCameraTestDisassemble.active then
		ControllerCameraTestExitDisassembleMode("Disassemble Mode Disabled", reason or "mode switch")
	elseif ControllerDisassembleBehavior then
		ControllerCameraTestDisassemble.doubleB = ControllerDisassembleBehavior.NewDoubleBTap()
	end
	if ControllerCameraTestAreaSelect.owner == "disassemble" then
		ControllerCameraTestCancelAreaSelect("mode switch")
	end
	local drag = ControllerCameraTestDragCommand
...
function ControllerCameraTestGetContextSnapshot()
	...
	local _, activeCommandDescs = ControllerCameraTestBuildActiveCommandLookup()
	local capabilitySignature = ControllerCameraTestActiveCommandSignature(activeCommandDescs)
	local capabilityCache = ControllerCameraTestCommandCapabilities
	if capabilityCache.selectionRevision ~= (ControllerCameraTestVisibleSelection.selectionRevision or 0)
			or capabilityCache.activeSignature ~= capabilitySignature then
		capabilityCache.selectionRevision = ControllerCameraTestVisibleSelection.selectionRevision or 0
		capabilityCache.activeSignature = capabilitySignature
		capabilityCache.values = ControllerCameraTestCommandCapabilitiesFromDescs(activeCommandDescs, #selected > 0)
	end
	local commandCaps = capabilityCache.values or {}
	local nativeCommandActive = ControllerCameraTestGetNativeActiveCommandID and
		ControllerCameraTestGetNativeActiveCommandID() ~= nil or false
	local nativeTargetingActive = ControllerCameraTestNativeTargetingIsActive
		and ControllerCameraTestNativeTargetingIsActive() or false
	local generalInsertAvailable = #selected > 0
		and ControllerCameraTestBuildPlacement.active ~= true
		and ControllerCameraTestBuildMenu.open ~= true
		and ControllerCameraTestDisassemble.active ~= true
		and (type(ControllerCameraTestTacticalMenu.stagedOption) == "table"
			or nativeCommandActive or nativeTargetingActive or commandCaps.canSmartAction == true)
	local canDisassemble = ControllerCameraTestSelectionHasDisassembleOperator(selected)
	local selfDestructEligible = #selected > 0
		and type(ControllerCameraTestFindSelfDestructCommandID(false)) == "number"
	local mobileTypeNavigationAvailable = ControllerCameraTestHasMobileUnitTypeBuckets
		and ControllerCameraTestHasMobileUnitTypeBuckets() or false
	return {
		...
		nativeCommandActive = nativeCommandActive,
		nativeTargetingActive = nativeTargetingActive,
		generalInsertAvailable = generalInsertAvailable,
		factoryQuotaMode = ControllerCameraTestBuildMenu.factoryQuotaModeActive == true,
		mobileTypeNavigationAvailable = mobileTypeNavigationAvailable,
		selfDestructFirstModifier = IsButtonDown("back"),
		selfDestructEligible = selfDestructEligible,
		...
		canBuildCommand = hasBuilder or hasFactory,
		canSmartAction = commandCaps.canSmartAction == true,
		canTacticalCommand = commandCaps.canTacticalCommand == true,
		canStopCommand = commandCaps.canStopCommand == true,
		canMoveCommand = commandCaps.canMoveCommand == true,
		canFightCommand = commandCaps.canFightCommand == true,
		canPatrolCommand = commandCaps.canPatrolCommand == true,
		canGuardCommand = commandCaps.canGuardCommand == true,
		canReclaimCommand = commandCaps.canReclaimCommand == true,
		canRepairCommand = commandCaps.canRepairCommand == true,
		canAttackCommand = commandCaps.canAttackCommand == true,
...
function ControllerCameraTestInstallWGAPI()
	...
	WG.BARControllerSupport.SyncNativeControlGroups = ControllerCameraTestSyncNativeControlGroups
	WG.BARControllerSupport.ReplaceNativeControlGroup = ControllerCameraTestAssignControlGroup
	WG.BARControllerSupport.AddSelectionToNativeControlGroup = ControllerCameraTestAddSelectionToControlGroup
	WG.BARControllerSupport.RemoveSelectionFromNativeControlGroup = ControllerCameraTestRemoveSelectionFromControlGroup
	WG.BARControllerSupport.SelectNativeControlGroup = ControllerCameraTestRecallControlGroup
	WG.BARControllerSupport.ClearNativeControlGroup = ControllerCameraTestClearControlGroup
end
...
function ControllerCameraTestRemoveWGAPI()
	...
	WG.BARControllerSupport.SyncNativeControlGroups = nil
	WG.BARControllerSupport.ReplaceNativeControlGroup = nil
	WG.BARControllerSupport.AddSelectionToNativeControlGroup = nil
	WG.BARControllerSupport.RemoveSelectionFromNativeControlGroup = nil
	WG.BARControllerSupport.SelectNativeControlGroup = nil
	WG.BARControllerSupport.ClearNativeControlGroup = nil
end
...
function ControllerCameraTestGetSettingsUICategories()
	return {
		...
		{ key = "Input", items = {
			...
			{ key = "rbTapMaxSeconds", label = "RB tap maximum", step = 0.01, decimals = 2 },
			{ key = "rbChordGraceSeconds", label = "RB-first chord grace", step = 0.01, decimals = 2 },
			{ key = "disassembleAreaReclaimRadius", label = "Disassemble Area Radius", step = 4, decimals = 0 },
		...
		{ key = "Radials", items = {
			...
			{ key = "nativePanelsDebugVisible", label = "Debug: keep native panels visible", type = "bool" },
		...
		{ key = "Selection", items = {
			...
			{ key = "selectionTransitionDebug", label = "Debug single-A transitions", type = "bool" },
		} },
		{ key = "Placement", items = {
...
function ControllerCameraTestIsQueueFrontModifierActive()
	if ControllerCameraTestBuildPlacement.active == true then
		return ControllerCameraTestIsInsertModifierActive()
	end
	return ControllerCameraTestGeneralInsert and ControllerCameraTestGeneralInsert.active == true
end
...
function ControllerCameraTestConsumeRBChord(reason)
	if ControllerInputChords and type(ControllerInputChords.Consume) == "function" then
		ControllerCameraTestInputChord = ControllerInputChords.Consume(
			ControllerCameraTestInputChord, tostring(reason or "RB chord"))
	end
	ControllerCameraTestTacticalToggleLatch.waitingForRBRelease = true
	ControllerCameraTestTacticalToggleLatch.lastResult = tostring(reason or "RB chord")
	ControllerCameraTestCancelVisibleSelectionRadial(tostring(reason or "RB chord"))
end
...
function ControllerCameraTestUpdateRTRBHardSuppression()
	...
	ControllerCameraTestConsumeRBChord("RT+RB suppression")
	return true
end
...
local function ControllerCameraTestIssueBuildOrders(builders, unitDefID, buildPositions, useQueue, useQueueFront)
	...
	local placement = ControllerCameraTestBuildPlacement
	if placement.distributedGridArmed and not useQueueFront then
		local api = WG["build_split"]
		if api and type(api.controllerDistribute) == "function" then
			local ok, distributed, builderCount, positionCount = pcall(
				api.controllerDistribute, -math.abs(unitDefID), buildPositions)
			placement.distributedGridArmed = false
			placement.distributedGridResult = ok and distributed
				and ("distributed " .. tostring(positionCount) .. " positions across " .. tostring(builderCount) .. " constructors")
				or (builderCount == 1 and "single constructor normal grid" or "native distribution unavailable")
			if ok and distributed then
				ControllerCameraTestCommandDebug.lastOptions = "distributed-shift"
				return true
			end
		else
			placement.distributedGridArmed = false
			placement.distributedGridResult = "Build Split API unavailable"
		end
	end
...
function ControllerCameraTestCancelDrag(reason)
	...
	drag.generalInsertPressActive = false
	if ControllerCameraTestGeneralInsert then ControllerCameraTestGeneralInsert.active = false end
...
function ControllerCameraTestFindSelfDestructCommandID(includeDisabled)
	local lookup, descs = ControllerCameraTestBuildActiveCommandLookup()
	if type(descs) == "table" then
		for _, desc in ipairs(descs) do
			local cmdID = desc and tonumber(desc.id or desc.cmdID)
			if type(cmdID) == "number" and (includeDisabled == true or not desc.disabled) then
				local text = ControllerCameraTestSmartDescText(desc)
				...
				then
					return cmdID, tostring(desc.action or desc.name or "activeCmdDesc"), desc.disabled == true, desc
				end
			end
		end
	end
	if type(CMD.SELFD) == "number" and (type(lookup) ~= "table" or lookup[CMD.SELFD]) then
		return CMD.SELFD, "CMD.SELFD", false, nil
	end
	if includeDisabled == true and type(CMD.SELFD) == "number" then
		return CMD.SELFD, "CMD.SELFD unavailable", true, nil
	end
	return nil, "unavailable", true, nil
end

function ControllerCameraTestEnsureSelfDestructCommand(commands)
	if type(commands) ~= "table" then return commands end
	local selectedUnits = type(spGetSelectedUnits) == "function" and spGetSelectedUnits() or {}
	if #selectedUnits == 0 then return commands end
	local found = false
	for _, option in ipairs(commands) do
		local text = ControllerCameraTestSmartDescText(option)
		if option.kind == "self_destruct"
...
local function tryNativeSmartRepairReclaimExtension(targetOverride)
	...
	if not (api and type(api.controllerExecuteAtTarget) == "function") then return false end
	...
	local accepted, route = api.controllerExecuteAtTarget(defaultCmdID, target,
		ControllerCameraTestGetCommandOptions(),
		ControllerCameraTestIsQueueFrontModifierActive() and "insert-front" or nil)
	if accepted ~= true then return false end
	local action = defaultCmdID == repairID and "repair" or "reclaim"
	ControllerCameraTestCommandDebug.defaultCmdID = tostring(defaultCmdID)
	ControllerCameraTestCommandDebug.defaultCmdName = tostring(defaultCmdName or action)
	ControllerCameraTestCommandDebug.issuedCmdID = tostring(defaultCmdID)
	ControllerCameraTestCommandDebug.issuedParamsCount = 1
	ControllerCameraTestCommandDebug.smartChosenAction = action
	ControllerCameraTestCommandDebug.smartActionSource = "narrow native " .. action .. " extension"
	ControllerCameraTestCommandDebug.smartExactTargetType = "unit"
	ControllerCameraTestCommandDebug.smartExactTargetID = tostring(target.targetID)
	ControllerCameraTestCommandDebug.lastResult = "issued once via " .. tostring(route)
	ControllerCameraTestShowHotkeyFeedback(string.upper(action), action == "reclaim" and "reclaim" or "utility")
	return true
end
...
function ControllerCameraTestTraceNativeTarget(event, detail)
	if not ControllerCameraTestSettings.debugPanelVisible then return end
	local state = ControllerCameraTestNativeTargeting
	local activeID = ControllerCameraTestGetNativeActiveCommandID and ControllerCameraTestGetNativeActiveCommandID() or nil
	local row = string.format("%s phase=%s active=%s cached=%s A=%s/%s Aowner=%s X=%s/%s Xowner=%s anchor=%s armed=%s %s",
		tostring(event), tostring(state.phase), tostring(activeID), tostring(state.cmdID),
		tostring(ControllerCameraTestActionDown("select")), tostring(ControllerCameraTestActionPressed("select")),
		"native-targeting",
		tostring(ControllerCameraTestActionDown("smartAction")), tostring(ControllerCameraTestActionPressed("smartAction")),
		"native-targeting", tostring(state.anchor ~= nil), tostring(state.confirmationArmed == true), tostring(detail or ""))
	state.traceRevision = (state.traceRevision or 0) + 1
	state.lastTrace = row
	state.trace[#state.trace + 1] = row
	while #state.trace > 16 do table.remove(state.trace, 1) end
	Spring.Echo("[Controller Native Target] " .. row)
end

function ControllerCameraTestCancelActiveCommandTargeting(reason, armRelease)
	local selectionBefore = ControllerCameraTestSafeSelectionSnapshot(
		type(spGetSelectedUnits) == "function" and spGetSelectedUnits() or {})
	local context = ControllerCameraTestNativeTargeting.context
	if type(Spring.SetActiveCommand) == "function" then pcall(Spring.SetActiveCommand, nil) end
	...
	if ControllerNativeTargeting then
		...
		ControllerCameraTestNativeTargeting.phase = "IDLE"
	end
	ControllerCameraTestNativeTargeting.descriptorDirty = false
	ControllerCameraTestNativeTargeting.context = nil
	ControllerCameraTestNativeTargeting.dispatchStarted = false
	if WG.smartareareclaim and type(WG.smartareareclaim.controllerClearHighlightedTargets) == "function" then
		pcall(WG.smartareareclaim.controllerClearHighlightedTargets)
	end
	if context == "disassemble-same-type" and ControllerCameraTestDisassemble then
		local area = ControllerCameraTestDisassemble.areaReclaim
		area.active, area.confirmArmed, area.waitingForNeutral, area.candidates = false, false, false, {}
	...
	ControllerCameraTestTraceNativeTarget("cancel", reason)
	latchSelectionDebugMessage(reason or "Targeting cancelled")
	return true
end

function ControllerCameraTestBeginHybridTargeting(descriptor, option, context, acceptCurrentPress)
	if not ControllerNativeTargeting or type(descriptor) ~= "table" then return false end
	local state = ControllerCameraTestNativeTargeting
	ControllerNativeTargeting.Reset(state, "hybrid target created")
	ControllerNativeTargeting.SetDescriptor(state, descriptor, CMDTYPE)
	if state.phase == ControllerNativeTargeting.IDLE or state.phase == ControllerNativeTargeting.BUILD_PLACEMENT then
		return false
	end
	state.option = option
	state.context = context or "native-command"
	state.dragMode = type(option) == "table" and option.dragMode or nil
	state.dispatchStarted = false
	state.finalDispatchStatus = "pending"
	state.descriptorDirty = false
	state.trace = state.trace or {}
	if acceptCurrentPress then
		state.freshInputObserved = true
		state.phase = state.targetMode == ControllerNativeTargeting.POINT_TARGETING
			and ControllerNativeTargeting.POINT_TARGETING or ControllerNativeTargeting.WAITING_FOR_ANCHOR_PRESS
	end
	if type(Spring.SetActiveCommand) == "function" then pcall(Spring.SetActiveCommand, nil) end
	ControllerCameraTestTraceNativeTarget("TARGET STATE CREATED", tostring(state.cmdID))
	ControllerCameraTestTraceNativeTarget("CONTROLLER OWNS SHAPE", tostring(state.targetMode))
	return true
end
...
function ControllerCameraTestRefreshNativeTargetingDescriptor(force)
	...
	if state.phase ~= (ControllerNativeTargeting and ControllerNativeTargeting.IDLE or "IDLE") then
		return state.descriptor
	end
	if not force and not state.descriptorDirty then return nil end
	state.descriptorDirty = false
	local descriptor
	if type(api.controllerGetActiveTargetDescriptor) == "function" then
		local ok, value = pcall(api.controllerGetActiveTargetDescriptor)
		if ok and type(value) == "table" then descriptor = value end
	end
	if descriptor then ControllerCameraTestBeginHybridTargeting(descriptor, nil, "active-command", false) end
	ControllerCameraTestTraceNativeTarget("descriptor-refresh", descriptor and "hybrid" or "no descriptor")
	return state.descriptor
end
...
function ControllerCameraTestShowNativeTargetPreview()
	...
	if not state.anchor or not state.current then return false end
	drag.active, drag.pressActive, drag.nativeControllerTargeting = true, false, true
	drag.mode = state.dragMode or (state.context == "disassemble-same-type" and "reclaimArea")
		or (state.shape == "area" and "genericArea") or tostring(state.shape or "nativeShape")
	drag.cmdID, drag.option = state.cmdID, state.option
...
function ControllerCameraTestIssueNativeTarget(params, shape)
	local state = ControllerCameraTestNativeTargeting
	if state.dispatchStarted then return false end
	...
	local api, accepted, route = WG and WG.ordermenu, false, "native adapter unavailable"
	local descriptor, cmdID = state.descriptor, tonumber(state.cmdID)
	ControllerCameraTestTraceNativeTarget("FINAL DISPATCH STARTED", tostring(shape))
	if state.context == "disassemble-same-type" and WG.smartareareclaim
			and type(WG.smartareareclaim.controllerCompleteArea) == "function" then
		accepted, route = WG.smartareareclaim.controllerCompleteArea(descriptor, "same",
			state.anchorTargetID, params[1], params[2], params[3], params[4], options, dispatchMode)
	elseif cmdID == tonumber((CMD and CMD.RECLAIM) or 90) and shape == "area"
			and WG.smartareareclaim and type(WG.smartareareclaim.controllerCompleteArea) == "function" then
		accepted, route = WG.smartareareclaim.controllerCompleteArea(descriptor, "any", nil,
			params[1], params[2], params[3], params[4], options, dispatchMode)
	elseif GameCMD and cmdID == tonumber(GameCMD.AREA_MEX) and WG.areamex
			and type(WG.areamex.controllerCompleteArea) == "function" then
		accepted, route = WG.areamex.controllerCompleteArea(descriptor,
			params[1], params[2], params[3], params[4], options, dispatchMode)
	elseif (shape == "front" or shape == "rectangle") and WG.customformations
			and type(WG.customformations.controllerCompleteShape) == "function" then
		accepted, route = WG.customformations.controllerCompleteShape(descriptor, shape, params, options, dispatchMode)
	elseif api and type(api.controllerCompleteTargetShape) == "function" then
		accepted, route = api.controllerCompleteTargetShape(descriptor, shape or "point", params, options, dispatchMode)
	end
	state.finalDispatchStatus = accepted == true and tostring(route or "accepted") or tostring(route or "rejected")
	ControllerCameraTestTraceNativeTarget(accepted == true and "FINAL DISPATCH ACCEPTED" or "FINAL DISPATCH REJECTED", route)
	if accepted == true then
		lastIssuedCommand = "Native target: " .. tostring(cmdID) .. " via " .. tostring(route)
		if dispatchMode == "insert-front" then
			ControllerCameraTestShowHotkeyFeedback("INSERT", "queue")
		end
		if state.context == "disassemble-same-type" then
			ControllerCameraTestDisassemble.successfulActivity = true
			ControllerCameraTestDisassemble.lastReclaimAt = debugEventTime
			ControllerCameraTestDisassemble.lastResult = "same-type reclaim via " .. tostring(route)
			ControllerCameraTestDisassemble.areaReclaim.active = false
			ControllerCameraTestDisassemble.areaReclaim.candidates = {}
			ControllerCameraTestRestoreDisassembleConstructors("Constructors restored")
			ControllerCameraTestShowHotkeyFeedback("RECLAIM", "reclaim")
		end
		ControllerCameraTestClearNativeTargetPreview("target dispatched")
		ControllerNativeTargeting.Reset(state, "target dispatched")
		state.descriptorDirty, state.dispatchStarted = false, false
		if WG.smartareareclaim and type(WG.smartareareclaim.controllerClearHighlightedTargets) == "function" then
			pcall(WG.smartareareclaim.controllerClearHighlightedTargets)
		end
	else
...
function ControllerCameraTestHandleNativeTargetingInput()
	if not ControllerCameraTestUsesNativeBARUI() then return false end
	local state = ControllerCameraTestNativeTargeting
	ControllerCameraTestRefreshNativeTargetingDescriptor(false)
	if state.phase == (ControllerNativeTargeting and ControllerNativeTargeting.IDLE or "IDLE")
			or state.phase == (ControllerNativeTargeting and ControllerNativeTargeting.BUILD_PLACEMENT or "BUILD_PLACEMENT") then
		return false
	end
	...
	if state.anchor then
		...
		if state.context == "disassemble-same-type" then
			local targets = ControllerCameraTestCollectOwnedTargetsInRadius(state.anchor.x, state.anchor.z,
				state.radius or 0, state.anchorUnitDefID and { [state.anchorUnitDefID] = true } or nil)
			ControllerCameraTestDisassemble.areaReclaim.candidates = targets
			local highlighted = {}
			for _, unitID in ipairs(targets) do highlighted[unitID] = true end
			if WG.smartareareclaim and type(WG.smartareareclaim.controllerSetHighlightedTargets) == "function" then
				pcall(WG.smartareareclaim.controllerSetHighlightedTargets, highlighted)
			end
		end
	end
	...
	local pressed = ControllerCameraTestActionPressed("select") or ControllerCameraTestActionPressed("smartAction")
	local generalInsertPressed = ControllerCameraTestIsRBXGeneralInsertPressed()
	if pressed and ControllerNativeTargeting.CanAcceptPress(state) then
		if state.anchor then
			local params, shape = ControllerNativeTargeting.BuildAnchoredParams(state)
			if params then
				if generalInsertPressed then
					ControllerCameraTestRunWithGeneralInsert("native target", function()
						return ControllerCameraTestIssueNativeTarget(params, shape)
					end)
				else
					ControllerCameraTestIssueNativeTarget(params, shape)
				end
			else ControllerCameraTestTraceNativeTarget("CONFIRM REJECTED", shape) end
		else
			local params, result = ControllerNativeTargeting.BeginOrBuildPoint(state, target, CMDTYPE)
			if params then
				if generalInsertPressed then
					ControllerCameraTestRunWithGeneralInsert("native target", function()
						return ControllerCameraTestIssueNativeTarget(params, "point")
					end)
				else
					ControllerCameraTestIssueNativeTarget(params, "point")
				end
			elseif result == "anchor" then
				if generalInsertPressed then
					ControllerCameraTestConsumeRBChord("RB+X native anchor")
					ControllerCameraTestGeneralInsert.lastResult = "native anchor armed by RB+X"
				end
				ControllerCameraTestShowNativeTargetPreview()
				ControllerCameraTestShowHotkeyFeedback("ANCHOR", "utility")
				ControllerCameraTestTraceNativeTarget("ANCHORED", tostring(state.shape))
			else
				ControllerCameraTestTraceNativeTarget("TARGET REJECTED", result)
			end
		end
	end
...
function ControllerCameraTestGetReticleNativeReclaimTarget()
	local target = ControllerCameraTestGetReticleTargetInfo()
	local unitID = target.targetType == "unit" and tonumber(target.targetID) or nil
	if not unitID then return nil, nil, nil end
	if type(Spring.ValidUnitID) == "function" and not Spring.ValidUnitID(unitID) then return nil, nil, nil end
	if type(Spring.GetUnitIsDead) == "function" and Spring.GetUnitIsDead(unitID) then return nil, nil, nil end
	local unitDefID, unitDef = ControllerCameraTestGetUnitDef(unitID)
	if not unitDefID or not unitDef then return nil, nil, nil end
	return unitID, unitDefID, unitDef
end

function ControllerCameraTestGetReticleNativeReclaimCommandTarget()
	local target = ControllerCameraTestGetReticleTargetInfo()
	local targetType, targetID = tostring(target.targetType or ""), tonumber(target.targetID)
	if targetType == "unit" and targetID then
		if ControllerCameraTestConstructorSet()[targetID] then return nil end
		if type(Spring.ValidUnitID) == "function" and not Spring.ValidUnitID(targetID) then return nil end
		if type(Spring.GetUnitIsDead) == "function" and Spring.GetUnitIsDead(targetID) then return nil end
		target.targetType, target.targetID = "unit", targetID
		target.hasWorld = false
		return target
	elseif targetType == "feature" and targetID then
		target.targetType, target.targetID = "feature", targetID
		target.hasWorld = false
		return target
	end
	return nil
...
function ControllerCameraTestIsDisassembleConstructor(unitID)
	if not ControllerCameraTestIsSafeSelectableUnit(unitID) then return false end
	local _, unitDef = ControllerCameraTestGetUnitDef(unitID)
	local constructorDef = ControllerDisassembleBehavior and ControllerDisassembleBehavior.IsConstructorDef(unitDef)
	local operatorDef = constructorDef == true
		or ControllerCameraTestIsCommanderUnit(unitID)
		or ControllerCameraTestIsBuilderUnitDef(unitDef)
	return operatorDef == true
		and ControllerCameraTestUnitHasReclaimCapability(unitID)
end

function ControllerCameraTestSelectionHasDisassembleOperator(selected)
	for _, unitID in ipairs(type(selected) == "table" and selected or {}) do
		if ControllerCameraTestIsDisassembleConstructor(unitID) then return true end
	end
	return false
end
...
function ControllerCameraTestIssueNativeDisassembleMove(targetOverride)
	ControllerCameraTestCancelNativeTargeting()
	local target = type(targetOverride) == "table" and targetOverride or nil
	local hasWorld = target and target.hasWorld or reticleHasWorldTarget
	local x = target and target.x or reticleWorldX
	local y = target and target.y or reticleWorldY
	local z = target and target.z or reticleWorldZ
	if not hasWorld or not x or not z then
	...
	local ok = ControllerCameraTestIssueOrderToSelectedUnits((CMD and CMD.MOVE) or 10,
		{ x, y or 0, z }, "Move", "ground", {})
	if ok then ControllerCameraTestShowHotkeyFeedback("MOVE", "utility") end
	return ok == true
end

function ControllerCameraTestAppendDisassembleReclaimer(unitID)
	if not ControllerCameraTestIsDisassembleConstructor(unitID) then return false end
	local state = ControllerCameraTestDisassemble
	for _, existing in ipairs(state.reclaimers or {}) do if existing == unitID then return true end end
	state.reclaimers[#state.reclaimers + 1] = unitID
	table.sort(state.reclaimers)
	state.lastResult = "touched constructor joined reclaimers"
	ControllerCameraTestRestoreDisassembleConstructors("Constructor joined reclaimers")
	return true
end

function ControllerCameraTestIssueDisassembleSingleReclaim(target, source)
	local state = ControllerCameraTestDisassemble
	target = type(target) == "table" and target or ControllerCameraTestGetReticleNativeReclaimCommandTarget()
	if not target or (target.targetType ~= "unit" and target.targetType ~= "feature") or not tonumber(target.targetID) then
		state.lastResult = tostring(source or "Disassemble") .. " reclaim skipped: no valid target"
		return false
	end
	...
	local option = { name = "Reclaim", cmdID = (CMD and CMD.RECLAIM) or 90, kind = "reclaim" }
	local ok = ControllerCameraTestExecuteTacticalCommand(option, false, nil, target)
	if ok then
		state.successfulActivity = true
		state.lastReclaimAt = debugEventTime
		state.lastResult = tostring(source or "Disassemble") .. " one-shot reclaim"
		ControllerCameraTestShowHotkeyFeedback("RECLAIM", "reclaim")
	else
		state.lastResult = tostring(source or "Disassemble") .. " reclaim rejected"
	end
	state.doubleB = ControllerDisassembleBehavior.NewDoubleBTap()
	return ok == true
end

function ControllerCameraTestIssueNativeDisassembleTarget()
	return ControllerCameraTestIssueDisassembleSingleReclaim(
		ControllerCameraTestGetReticleNativeReclaimCommandTarget(), "Native Disassemble")
end

function ControllerCameraTestHandleDisassembleXInput(dt)
	local state = ControllerCameraTestDisassemble
	state.pendingX = type(state.pendingX) == "table" and state.pendingX
		or { pressActive = false, startedAt = 0, holdFired = false, targetInfo = nil, groundInfo = nil }
	local pending, drag = state.pendingX, ControllerCameraTestDragCommand
	local holdSeconds = ControllerCameraTestSettings.xHoldSeconds or 0.14

	if ControllerCameraTestActionPressed("smartAction") then
		pending.pressActive = true
		pending.startedAt = debugEventTime
		pending.holdFired = false
		pending.targetInfo = ControllerCameraTestGetReticleNativeReclaimCommandTarget()
		pending.groundInfo = ControllerCameraTestGetReticleTargetInfo()
		drag.pressActive = true
		drag.pressStartTime = debugEventTime
		drag.pressButton = "disassemble-smartAction"
		drag.startX, drag.startY, drag.startZ = reticleWorldX, reticleWorldY, reticleWorldZ
		drag.endX, drag.endY, drag.endZ = reticleWorldX, reticleWorldY, reticleWorldZ
		drag.active = false
		drag.singleUnitPathActive = false
	...
	if pending.pressActive and ControllerCameraTestActionDown("smartAction") then
		if not pending.holdFired and debugEventTime - (pending.startedAt or debugEventTime) >= holdSeconds then
			...
			if #selectedUnits == 1 and #mobileUnits == 1 and reticleHasWorldTarget then
				...
				latchSelectionDebugMessage("moveLine Drag started")
			end
		end
		if pending.holdFired and drag.active and drag.mode ~= "singleMovePath" then
			ControllerCameraTestUpdateDragPreview()
		end
		return true
	end

	if pending.pressActive and ControllerCameraTestActionReleased("smartAction") then
		if pending.holdFired then
			if drag.singleUnitPathActive then
			...
			if pending.targetInfo then
				ControllerCameraTestIssueDisassembleSingleReclaim(pending.targetInfo, "Disassemble X")
			else
				ControllerCameraTestIssueNativeDisassembleMove(pending.groundInfo)
				state.doubleB = ControllerDisassembleBehavior.NewDoubleBTap()
			end
		end
		pending.pressActive, pending.holdFired, pending.targetInfo, pending.groundInfo = false, false, nil, nil
		drag.pressActive = false
		if drag.pressButton == "disassemble-smartAction" then drag.pressButton = nil end
		return true
	end

	return pending.pressActive or (drag.active and drag.pressButton == "disassemble-smartAction")
end
...
function ControllerCameraTestGetDisassembleAreaReclaimRadius()
	return ControllerCameraTestClampSetting("disassembleAreaReclaimRadius",
		ControllerCameraTestSettings.disassembleAreaReclaimRadius or 24)
end

function ControllerCameraTestBeginNativeReclaim(mode, targetID, unitDefID)
	if not targetID then return false end
	if type(Spring.ValidUnitID) == "function" and not Spring.ValidUnitID(targetID) then return false end
	if type(Spring.GetUnitIsDead) == "function" and Spring.GetUnitIsDead(targetID) then return false end
	local x, y, z = spGetUnitPosition(targetID)
	local orderApi = WG and WG.ordermenu
	if not x or not z or not WG.smartareareclaim or not orderApi
			or type(orderApi.controllerGetCommandDescriptor) ~= "function" then
		return false
	end
	local descriptor = orderApi.controllerGetCommandDescriptor((CMD and CMD.RECLAIM) or 90)
	if type(descriptor) ~= "table" then return false end
	local radius = mode == "same" and ControllerCameraTestGetDisassembleAreaReclaimRadius() or nil
	local state = ControllerCameraTestDisassemble
	state.areaReclaim = { active = mode == "same", anchorUnitID = targetID, unitDefID = unitDefID,
		x = x, y = y or 0, z = z, radius = radius or 0, candidates = {}, descriptor = descriptor, mode = mode }
	state.lastResult = "native " .. tostring(mode) .. " reclaim active"
	return true
end

function ControllerCameraTestConfirmNativeReclaim()
	if not WG.smartareareclaim or type(WG.smartareareclaim.controllerCompleteArea) ~= "function" then
		return false
	end
	local area = ControllerCameraTestDisassemble.areaReclaim
	local ok, result = pcall(WG.smartareareclaim.controllerCompleteArea,
		area.descriptor, area.mode or "single", area.anchorUnitID,
...
function ControllerCameraTestConfirmNativeActiveCommand(cmdID)
	...
	if cmdID == ((CMD and CMD.RECLAIM) or 90) and target.targetType == "unit" then
		local targetID, unitDefID = ControllerCameraTestGetReticleNativeReclaimTarget()
...
function ControllerCameraTestIsRBXGeneralInsertPressed()
	return ControllerCameraTestActionDown("buildRadial")
		and ControllerCameraTestActionPressed("smartAction")
		and not ControllerCameraTestBuildMenu.open
		and not ControllerCameraTestBuildPlacement.active
		and not ControllerCameraTestDisassemble.active
		and not commandLayerActive
end

function ControllerCameraTestRunWithGeneralInsert(source, issueFn)
	if type(issueFn) ~= "function" then return false end
	local state = ControllerCameraTestGeneralInsert
	state.active = true
	state.lastSource = tostring(source or "general")
	state.lastResult = "front insert active"
	ControllerCameraTestConsumeRBChord("RB+X general insert")
	local ok, result = pcall(issueFn)
	state.active = false
	state.lastIssued = ok and result == true
	state.lastResult = state.lastIssued and ("inserted via " .. state.lastSource)
		or ("insert rejected via " .. state.lastSource)
	if not ok then
		state.lastResult = "insert error: " .. tostring(result)
		if ControllerCameraTestSettings.debugPanelVisible then
			Spring.Echo("[Controller Insert] " .. state.lastResult)
		end
	end
	return ok and result == true
end

function ControllerCameraTestNativeTargetingIsActive()
	local phase = ControllerCameraTestNativeTargeting and ControllerCameraTestNativeTargeting.phase
	return phase ~= nil and phase ~= "IDLE" and phase ~= "BUILD_PLACEMENT"
end

function ControllerCameraTestUpdateRBXGeneralInsert()
	if not ControllerCameraTestIsRBXGeneralInsertPressed() then return false end
	if type(ControllerCameraTestTacticalMenu.stagedOption) == "table"
			or ControllerCameraTestNativeTargetingIsActive() then
		return false
	end
	local cmdID = ControllerCameraTestGetNativeActiveCommandID and ControllerCameraTestGetNativeActiveCommandID() or nil
	if type(cmdID) ~= "number" then return false end
	return ControllerCameraTestRunWithGeneralInsert("native active command", function()
		return ControllerCameraTestConfirmNativeActiveCommand(cmdID)
	end)
end
...
function ControllerCameraTestNativeDisassembleTargets(units, unitDefID)
	local function validTarget(unitID)
		...
		return type(Spring.GetUnitIsDead) ~= "function" or not Spring.GetUnitIsDead(unitID)
	end
	return ControllerDisassembleBehavior.FilterNativeTargets(units, ControllerCameraTestConstructorSet(),
...
function ControllerCameraTestHandleDisassembleB()
	local state = ControllerCameraTestDisassemble
	if ControllerCameraTestActionPressed("cancel") then
		local consumed = false
		local sharedArea = ControllerCameraTestAreaSelect
		if sharedArea and sharedArea.owner == "disassemble" and (sharedArea.pressActive or sharedArea.active) then
			ControllerCameraTestCancelAreaSelect("Disassemble selection cancelled")
			ControllerCameraTestRestoreDisassembleConstructors("Constructors restored")
			consumed = true
		elseif state.areaReclaim.active or state.lbA.pressActive or state.markArea.pressActive
				or state.markArea.active or (state.pendingX and state.pendingX.pressActive) then
	...
	local event
	event, state.doubleB = ControllerDisassembleBehavior.UpdateDoubleBTap(state.doubleB, debugEventTime,
		ControllerCameraTestActionPressed("cancel"), ControllerCameraTestActionReleased("cancel"),
		ControllerDisassembleBehavior.DISASSEMBLE_DOUBLE_B_SECONDS)
	if event == "exit" then
		ControllerCameraTestExitDisassembleMode("Disassemble Mode Disabled", "double-B exit")
		return true
	end
	if event ~= "first" then return false end
	state.lastResult = "first B registered"
	return true
end
...
function ControllerCameraTestStartNativeSameTypeReclaim(targetID, unitDefID)
	...
	area.radius = ControllerCameraTestGetDisassembleAreaReclaimRadius()
	area.candidates = {}
...
function ControllerCameraTestUpdateNativeDisassembleInput(dt)
	...
	if lbDown and ControllerCameraTestActionPressed("select") then
		...
		state.lbA = { pressActive = true, startedAt = debugEventTime,
			targetID = targetID, unitDefID = unitDefID, targetInfo = targetInfo, holdFired = false }
		return true
	end
...
function ControllerCameraTestCancelDisassembleSubstates(reason)
	...
	if ControllerCameraTestAreaSelect.owner == "disassemble" then
		ControllerCameraTestCancelAreaSelect(reason or "Disassemble selection cancelled")
	end
	state.markArea.pressActive, state.markArea.active = false, false
	...
	state.lbA.pressActive, state.lbA.holdFired, state.lbA.targetID, state.lbA.targetInfo = false, false, nil, nil
	if ControllerCameraTestDragCommand.pressButton == "disassemble-smartAction" then
		ControllerCameraTestCancelDrag(reason or "Disassemble X cancelled")
	end
	state.pendingX = { pressActive = false, startedAt = 0, holdFired = false, targetInfo = nil, groundInfo = nil }
...
function ControllerCameraTestEnterDisassembleMode()
	...
	if ControllerCameraTestUsesNativeBARUI() then
		...
		state.activatedAt, state.lastReclaimAt, state.successfulActivity, state.lastValidationAt =
			debugEventTime, debugEventTime, false, debugEventTime
		state.doubleB = ControllerDisassembleBehavior.NewDoubleBTap()
...
function ControllerCameraTestExitDisassembleMode(toast, reason)
	...
	if #reclaimers > 0 and type(spSelectUnitArray) == "function" then
		ControllerCameraTestVisibleSelection.suppressNextSnapshot = true
		pcall(spSelectUnitArray, reclaimers, false)
	end
...
function ControllerCameraTestCollectOwnedTargetsInRadius(x, z, radius, typeFilter)
	...
	if type(Spring.GetUnitsInCylinder) == "function" and x and z and radius then
		...
		local ok, units = pcall(Spring.GetUnitsInCylinder, x, z, radius)
		if ok and type(units) == "table" then
			for _, unitID in ipairs(units) do
	...
	if ControllerDisassembleBehavior then return ordered end
...
function ControllerCameraTestSyncDisassembleTargetHighlights(extraTargets)
	...
	for unitID, marked in pairs(type(extraTargets) == "table" and extraTargets or {}) do
		if marked then add(unitID) end
	end
	ControllerCameraTestDisassemble.highlightedTargets = highlighted
	if WG.smartareareclaim and type(WG.smartareareclaim.controllerSetHighlightedTargets) == "function" then
...
function ControllerCameraTestUpdateDisassembleAreaRadius(area, dt)
	...
	if delta ~= 0 then area.radius = clamp((area.radius or 320) + delta,
		ControllerDisassembleBehavior and ControllerDisassembleBehavior.MIN_TARGET_RADIUS or 12,
...
function ControllerCameraTestIssueDisassembleReclaim(targets)
	...
	for _, unitID in ipairs(type(ordered) == "table" and ordered or {}) do
		local valid = type(Spring.ValidUnitID) ~= "function" or Spring.ValidUnitID(unitID)
		if valid and type(Spring.GetUnitIsDead) == "function" then valid = not Spring.GetUnitIsDead(unitID) end
		if not constructors[unitID] and valid then
			validTargets[#validTargets + 1] = unitID
		end
	end
	local routes = ControllerDisassembleBehavior and ControllerDisassembleBehavior.PlanReclaimRoutes(
		state.reclaimers, validTargets, spGetUnitPosition) or {}
	local issuedTargets, reclaimID = 0, (CMD and CMD.RECLAIM) or 90
	if type(spGiveOrderToUnit) == "function" then
		for _, route in ipairs(routes) do
			for index, targetID in ipairs(route.targets) do
				local options = index == 1 and {} or { "shift" }
				local ok, result = pcall(spGiveOrderToUnit, route.constructorID,
					reclaimID, { targetID }, options)
				if ok and result ~= false then issuedTargets = issuedTargets + 1 end
			end
		end
	end
...
function ControllerCameraTestStartAreaReclaim(targetID, unitDefID)
	...
	state.areaReclaim = { active = true, anchorUnitID = targetID, unitDefID = unitDefID,
		...
		radius = ControllerCameraTestGetDisassembleAreaReclaimRadius(),
...
function ControllerCameraTestUpdateAreaReclaimTargeting()
	...
	if reticleHasWorldTarget and reticleWorldX and reticleWorldZ and not area.waitingForNeutral then
		local dx, dz = reticleWorldX - area.x, reticleWorldZ - area.z
		area.radius = clamp(math.sqrt(dx * dx + dz * dz), 12,
			ControllerDisassembleBehavior and ControllerDisassembleBehavior.MAX_TARGET_RADIUS or 1200)
		ControllerCameraTestSettings.disassembleAreaReclaimRadius = area.radius
	end
...
function ControllerCameraTestUpdateDisassembleModeInput(dt)
	...
	if lbDown and ControllerCameraTestActionPressed("select") then
		local targetInfo = ControllerCameraTestGetReticleNativeReclaimCommandTarget()
		local targetID, unitDefID = ControllerCameraTestGetReticleNativeReclaimTarget()
		state.lbA = { pressActive = targetInfo ~= nil or targetID ~= nil, startedAt = debugEventTime, targetID = targetID,
			unitDefID = unitDefID, targetInfo = targetInfo, holdFired = false }
		return true
	end
...
function ControllerCameraTestUpdateDisassembleLifecycle()
	...
	if debugEventTime - (state.lastValidationAt or -10) >= 0.25 then
		...
		if not ControllerCameraTestValidateReclaimers(not ControllerCameraTestUsesNativeBARUI()) then return end
	end
	if ControllerDisassembleBehavior and ControllerDisassembleBehavior.TimeoutExpired(
			state.lastReclaimAt or state.activatedAt, debugEventTime,
			ControllerDisassembleBehavior.DISASSEMBLE_INACTIVITY_TIMEOUT_SECONDS) then
		ControllerCameraTestExitDisassembleMode("Disassemble Mode Timed Out", "inactivity timeout")
	end
end
...
function ControllerCameraTestFactoryHasQueueOrProduction(unitID)
	if type(Spring.GetUnitIsBuilding) == "function" then
		local ok, buildingID = pcall(Spring.GetUnitIsBuilding, unitID)
		if ok and buildingID then return true end
	end
	if type(Spring.GetFactoryCommands) == "function" then
		local ok, queue = pcall(Spring.GetFactoryCommands, unitID, -1)
		if ok and type(queue) == "table" and #queue > 0 then return true end
	end
	return false
end
...
function ControllerCameraTestIsIdleCycleCandidate(unitID, requestedPool)
	...
	if isFactory and ControllerCameraTestFactoryHasQueueOrProduction(unitID) then
		return false, isBuilder, "none"
	end
	local pool = (isBuilder or isFactory or ControllerCameraTestIsCommanderUnit(unitID)) and "primary" or nil
...
function ControllerCameraTestIsMobileTypeNavigationCandidate(unitID)
	if not ControllerCameraTestIsSafeSelectableUnit(unitID) then return false end
	local _, unitDef = ControllerCameraTestGetUnitDef(unitID)
	if type(unitDef) ~= "table" then return false end
	if unitDef.isFactory == true or unitDef.isBuilding == true then return false end
	return ControllerCameraTestIsMobileUnitDef(unitDef)
end

function ControllerCameraTestGetMobileUnitTypeBuckets()
	local bucketsByDef, buckets = {}, {}
	for _, unitID in ipairs(ControllerCameraTestGetOwnTeamUnits()) do
		if ControllerCameraTestIsMobileTypeNavigationCandidate(unitID) then
			local unitDefID = type(Spring.GetUnitDefID) == "function" and Spring.GetUnitDefID(unitID) or nil
			if unitDefID then
	...
	for _, bucket in ipairs(buckets) do
		table.sort(bucket.units)
		bucket.representativeID = bucket.units[1]
	end
	table.sort(buckets, function(a, b)
		return (tonumber(a.representativeID) or math.huge) < (tonumber(b.representativeID) or math.huge)
	end)
	return buckets
end

function ControllerCameraTestHasMobileUnitTypeBuckets()
	return #ControllerCameraTestGetMobileUnitTypeBuckets() > 0
end

function ControllerCameraTestSelectRepresentativeFromMobileTypeBucket(bucket)
	if type(bucket) ~= "table" or not bucket.representativeID then return false end
	local state = ControllerCameraTestUnitTypeNavigation
	state.currentUnitDefID = bucket.unitDefID
	state.lastTypeName = bucket.name
	state.lastCount = #(bucket.units or {})
	state.representativeRule = "lowest-unit-id"
	if ControllerCameraTestFocusAndSelectUnit(bucket.representativeID, "Mobile type " .. tostring(bucket.name)) then
		state.lastResult = "mobile type representative " .. tostring(bucket.representativeID)
		ControllerCameraTestLayerDebug.normalUtilityAction = "Mobile type " .. tostring(bucket.name)
		return true
	end
	state.lastResult = "mobile type focus failed"
	return false
end

function ControllerCameraTestCycleMobileUnitType(delta)
	local buckets = ControllerCameraTestGetMobileUnitTypeBuckets()
	local state = ControllerCameraTestUnitTypeNavigation
	if #buckets == 0 then
		state.lastResult = "no mobile unit types"
		state.lastCount = 0
		latchSelectionDebugMessage("Mobile type: no owned mobile units")
		return false
	end
	local currentIndex = 0
	for index, bucket in ipairs(buckets) do
		if bucket.unitDefID == state.currentUnitDefID then
			currentIndex = index
			break
		end
	end
	local nextIndex = ((currentIndex - 1 + (tonumber(delta) or 1)) % #buckets) + 1
	state.currentIndex = nextIndex
	local bucket = buckets[nextIndex]
	if ControllerCameraTestSelectRepresentativeFromMobileTypeBucket(bucket) then
		state.lastResult = "mobile type " .. tostring(nextIndex) .. "/" .. tostring(#buckets)
		latchSelectionDebugMessage("Mobile type: " .. tostring(bucket.name))
		return true
	end
	return false
end

function ControllerCameraTestSelectVisibleMobileUnitType()
	local state = ControllerCameraTestUnitTypeNavigation
	local unitDefID = state.currentUnitDefID
	if not unitDefID then
		local buckets = ControllerCameraTestGetMobileUnitTypeBuckets()
		if buckets[1] then unitDefID = buckets[1].unitDefID end
	end
	if not unitDefID then
		state.selectedVisibleCount = 0
		state.lastResult = "no mobile type selected"
		return false
	end
	local units = {}
	local visibleSet, hasVisibleList = {}, false
	...
	for _, unitID in ipairs(ControllerCameraTestGetOwnTeamUnits()) do
		if (not hasVisibleList or visibleSet[unitID])
				and ControllerCameraTestIsMobileTypeNavigationCandidate(unitID)
				and type(Spring.GetUnitDefID) == "function"
				and Spring.GetUnitDefID(unitID) == unitDefID
				and ControllerCameraTestUnitIsOnScreen(unitID) then
			units[#units + 1] = unitID
		end
	end
	...
	if #units == 0 then
		state.selectedVisibleCount = 0
		state.lastResult = "no visible mobile type units"
		latchSelectionDebugMessage("Mobile type: no visible units")
		return false
	end
	local selected = ControllerCameraTestSelectUnits(units, "Visible mobile type")
	state.currentUnitDefID = unitDefID
	state.lastTypeName = ControllerCameraTestUnitTypeName(unitDefID)
	state.selectedVisibleCount = selected and #units or 0
	state.lastCount = #units
	state.lastResult = selected and ("selected visible mobile type x" .. tostring(#units)) or "visible type select failed"
	if selected then ControllerCameraTestFocusUnitsCenter(units, "Visible mobile type") end
	return selected
end

function ControllerCameraTestUpdateRBMobileTypeNavigation()
	if not ControllerCameraTestActionDown("buildRadial") then return false end
	if ControllerCameraTestBuildMenu.open or ControllerCameraTestBuildPlacement.active
			or ControllerCameraTestTacticalMenu.open
			or type(ControllerCameraTestTacticalMenu.stagedOption) == "table"
			or ControllerCameraTestDisassemble.active
			or ControllerCameraTestNativeTargetingIsActive()
			or commandLayerActive
			or ControllerCameraTestIsGameplayInputBlocked() then
		return false
	end
	local handled = false
	if WasButtonPressed("dpadLeft") then
		handled = ControllerCameraTestCycleMobileUnitType(-1)
	elseif WasButtonPressed("dpadRight") then
		handled = ControllerCameraTestCycleMobileUnitType(1)
	elseif WasButtonPressed("dpadDown") then
		handled = ControllerCameraTestSelectVisibleMobileUnitType()
	end
	if handled then
		ControllerCameraTestConsumeRBChord("RB+D-pad mobile type")
		ControllerCameraTestShowHotkeyFeedback("TYPE", "utility")
		return true
	end
	return false
end
...
function ControllerCameraTestSelectAllFocusedIdleType()
...
function ControllerCameraTestNativeGroupNumber(slot)
	return ControllerCameraTestNormalizeControlGroupSlot(slot) % 10
end

function ControllerCameraTestSyncNativeControlGroups()
	local groups = ControllerCameraTestControlGroups
	if type(Spring.GetGroupUnits) ~= "function" then return false end
	for slot = 1, 10 do
		local units = Spring.GetGroupUnits(ControllerCameraTestNativeGroupNumber(slot)) or {}
		units = ControllerCameraTestFilterValidUnits(units)
		table.sort(units)
		if #units > 0 then
			local unitDefID = type(Spring.GetUnitDefID) == "function" and Spring.GetUnitDefID(units[1]) or nil
			groups.slots[slot] = {
				units = units, count = #units, unitDefID = unitDefID,
				typeName = unitDefID and ControllerCameraTestUnitTypeName(unitDefID) or "Native group",
				native = true,
			}
		else
			groups.slots[slot] = nil
	...
	groups.nativeAutogroupStatus = "engine groups 0-9 authoritative"
	return true
end

function ControllerCameraTestPruneDeadUnitsFromControlGroup(slot)
	ControllerCameraTestSyncNativeControlGroups()
	local entry = ControllerCameraTestControlGroups.slots[ControllerCameraTestNormalizeControlGroupSlot(slot)]
	return entry and entry.units or {}
end

function ControllerCameraTestAssignControlGroup(slot)
	slot = ControllerCameraTestNormalizeControlGroupSlot(slot)
	if type(Spring.SetUnitGroup) ~= "function" or type(Spring.GetGroupUnits) ~= "function" then return false end
	local groupNumber = ControllerCameraTestNativeGroupNumber(slot)
	local selected = ControllerCameraTestFilterValidUnits(type(spGetSelectedUnits) == "function" and spGetSelectedUnits() or {})
	if #selected == 0 then
		ControllerCameraTestSetControlGroupAction("assign failed: no selected units", slot, 0)
		return false
	end
	for _, unitID in ipairs(Spring.GetGroupUnits(groupNumber) or {}) do Spring.SetUnitGroup(unitID, -1) end
	for _, unitID in ipairs(selected) do Spring.SetUnitGroup(unitID, groupNumber) end
	ControllerCameraTestSyncNativeControlGroups()
	ControllerCameraTestControlGroups.activeSlot = slot
	ControllerCameraTestSetControlGroupAction("replaced native group x" .. tostring(#selected), slot, #selected)
	return true
end

function ControllerCameraTestAddSelectionToControlGroup(slot)
	slot = ControllerCameraTestNormalizeControlGroupSlot(slot)
	if type(Spring.SetUnitGroup) ~= "function" then return false end
	local selected = ControllerCameraTestFilterValidUnits(type(spGetSelectedUnits) == "function" and spGetSelectedUnits() or {})
	for _, unitID in ipairs(selected) do Spring.SetUnitGroup(unitID, ControllerCameraTestNativeGroupNumber(slot)) end
	ControllerCameraTestSyncNativeControlGroups()
	ControllerCameraTestSetControlGroupAction("added selection", slot, #selected)
	return #selected > 0
end

function ControllerCameraTestRemoveSelectionFromControlGroup(slot)
	slot = ControllerCameraTestNormalizeControlGroupSlot(slot)
	if type(Spring.SetUnitGroup) ~= "function" or type(Spring.GetUnitGroup) ~= "function" then return false end
	local groupNumber, removed = ControllerCameraTestNativeGroupNumber(slot), 0
	for _, unitID in ipairs(type(spGetSelectedUnits) == "function" and spGetSelectedUnits() or {}) do
		if Spring.GetUnitGroup(unitID) == groupNumber then Spring.SetUnitGroup(unitID, -1); removed = removed + 1 end
	end
	ControllerCameraTestSyncNativeControlGroups()
	ControllerCameraTestSetControlGroupAction("removed selection", slot, removed)
	return removed > 0
end
...
function ControllerCameraTestRecallControlGroup(slot)
	...
	local units = ControllerCameraTestPruneDeadUnitsFromControlGroup(slot)
	ControllerCameraTestControlGroups.activeSlot = slot
	if #units == 0 then ControllerCameraTestSetControlGroupAction("empty", slot, 0); return false end
	if not ControllerCameraTestSelectUnits(units, "Native group " .. ControllerCameraTestGetControlGroupDisplaySlot(slot)) then return false end
	ControllerCameraTestFocusUnitsCenter(units, "Native control group")
	ControllerCameraTestSetControlGroupAction("recalled native group x" .. tostring(#units), slot, #units)
	return true
end

function ControllerCameraTestClearControlGroup(slot)
	slot = ControllerCameraTestNormalizeControlGroupSlot(slot)
	if type(Spring.GetGroupUnits) ~= "function" or type(Spring.SetUnitGroup) ~= "function" then return false end
	for _, unitID in ipairs(Spring.GetGroupUnits(ControllerCameraTestNativeGroupNumber(slot)) or {}) do Spring.SetUnitGroup(unitID, -1) end
	ControllerCameraTestSyncNativeControlGroups()
	ControllerCameraTestControlGroups.activeSlot = slot
	ControllerCameraTestSetControlGroupAction("cleared native group", slot, 0)
	return true
end

function ControllerCameraTestAutoAddFinishedUnitToGroups()
	-- Native groups never auto-add future units; keyboard and controller group
	-- membership remain one authoritative engine data set.
	ControllerCameraTestSyncNativeControlGroups()
end

function ControllerCameraTestRemoveUnitFromControlGroups()
	ControllerCameraTestSyncNativeControlGroups()
end
...
function ControllerCameraTestActiveCommandSignature(descs)
	if type(descs) ~= "table" then return "no-descs" end
	local parts = {}
	for index, desc in ipairs(descs) do
		parts[index] = tostring(desc and (desc.id or desc.cmdID) or "nil")
			.. ":" .. tostring(desc and desc.disabled == true)
	end
	return table.concat(parts, ",")
end

function ControllerCameraTestCommandCapabilitiesFromDescs(descs, hasSelection)
	local unknown = type(descs) ~= "table"
	local values = {
		canStopCommand = unknown and hasSelection or false,
		canMoveCommand = unknown and hasSelection or false,
		canFightCommand = unknown and hasSelection or false,
		canPatrolCommand = unknown and hasSelection or false,
		canGuardCommand = unknown and hasSelection or false,
		canReclaimCommand = unknown and hasSelection or false,
		canRepairCommand = unknown and hasSelection or false,
		canAttackCommand = unknown and hasSelection or false,
		canSmartAction = unknown and hasSelection or false,
		canTacticalCommand = unknown and hasSelection or false,
	}
	if unknown then return values end
	local function mark(name)
		values[name] = true
		values.canTacticalCommand = true
	end
	for _, desc in ipairs(descs) do
		if desc and desc.disabled ~= true then
			local cmdID = tonumber(desc.id or desc.cmdID)
			local text = ControllerCameraTestSmartDescText(desc)
			if cmdID == CMD.STOP or text:find("stop", 1, true) then mark("canStopCommand") end
			if cmdID == CMD.MOVE or text:find("move", 1, true) then mark("canMoveCommand") end
			if cmdID == CMD.FIGHT or text:find("fight", 1, true) then mark("canFightCommand") end
			if cmdID == CMD.PATROL or text:find("patrol", 1, true) then mark("canPatrolCommand") end
			if cmdID == CMD.GUARD or text:find("guard", 1, true) then mark("canGuardCommand") end
			if cmdID == CMD.RECLAIM or text:find("reclaim", 1, true) then mark("canReclaimCommand") end
			if cmdID == CMD.REPAIR or text:find("repair", 1, true) then mark("canRepairCommand") end
			if cmdID == CMD.ATTACK or text:find("attack", 1, true) then mark("canAttackCommand") end
		end
	end
	values.canSmartAction = values.canMoveCommand or values.canFightCommand or values.canPatrolCommand
		or values.canGuardCommand or values.canReclaimCommand or values.canRepairCommand or values.canAttackCommand
	values.canTacticalCommand = values.canTacticalCommand or values.canSmartAction or values.canStopCommand
	return values
end
...
function ControllerCameraTestRebuildNativeTacticalModel(reason)
	...
	for index, command in ipairs(commands) do nativeCommands[index] = command end
	ControllerCameraTestEnsureSelfDestructCommand(nativeCommands)
	local model = ControllerCameraTestNativeRadialAdapter:BuildTacticalModel(nativeCommands, {
...
function ControllerCameraTestRebuildTacticalCommandCache(reason)
	...
	if isFactory then
		...
		ControllerCameraTestEnsureSelfDestructCommand(commands)
...
function ControllerCameraTestHandlePriorityTacticalToggle()
	local latch = ControllerCameraTestTacticalToggleLatch
	if latch.waitingForRBRelease then
		if not ControllerCameraTestActionDown("buildRadial") then
			latch.waitingForRBRelease = false
			latch.lastResult = "rearmed after RB release"
		end
		return true
	end
	if not IsButtonDown("back") or not ControllerCameraTestTacticalTogglePressed() then return false end
	if ControllerCameraTestBuildMenu.open then
		ControllerCameraTestCloseBuildMenu("closed for Tactical Radial")
	end
	ControllerCameraTestToggleTacticalMenu()
	latch.waitingForRBRelease = true
	latch.lastResult = ControllerCameraTestTacticalMenu.open and "opened Tactical Radial" or "closed Tactical Radial"
	if ControllerInputChords then
		ControllerCameraTestInputChord = ControllerInputChords.Consume(
			ControllerCameraTestInputChord, "Back+RB Tactical toggle")
	end
	ControllerCameraTestTuning.backCommandLayerATapTime = -10
	ControllerCameraTestLayerDebug.commandLayerAction = latch.lastResult
	return true
end
		...
		if isAreaCmd or isLineCmd then
			if not drag.active then
				if reticleHasWorldTarget and reticleWorldX then
					if generalInsertPressed then
						ControllerCameraTestConsumeRBChord("RB+X staged anchor")
						ControllerCameraTestGeneralInsert.lastResult = "staged anchor armed by RB+X"
					end
					drag.startX, drag.startY, drag.startZ = reticleWorldX, reticleWorldY, reticleWorldZ
					drag.endX, drag.endY, drag.endZ = reticleWorldX, reticleWorldY, reticleWorldZ
					drag.active = true
				...
				if generalInsertPressed then
					ControllerCameraTestRunWithGeneralInsert("staged tactical", function()
						return ControllerCameraTestConfirmStagedTacticalCommand()
					end)
				else
...
function ControllerCameraTestExecuteTacticalCommand(option, stageTargeted, stateDelta, targetOverride)
	if type(option) ~= "table" then
	...
	if ControllerCameraTestUsesNativeBARUI()
		...
		if option.isState and stageTargeted then
			...
			elseif type(option.states) == "table" and #option.states >= 3 then
				local count = #option.states
				local desired = ((tonumber(option.currentStateIndex) or 0)
					+ ((tonumber(stateDelta) or 1) < 0 and -1 or 1)) % count
				return ControllerCameraTestActivateNativeState(option, desired)
			end
		end
		local api = WG and WG.ordermenu
...
function ControllerCameraTestHandleTacticalMenuInput()
	...
	elseif ControllerCameraTestActionPressed("tacticalSelect") or ControllerCameraTestActionPressed("radialQuick") then
		...
		local stateDelta = ControllerCameraTestActionPressed("radialQuick") and -1 or 1
		ControllerCameraTestExecuteTacticalCommand(commands[menu.selectedIndex], true, stateDelta)
		changed = true
	end
	if changed then
		ControllerCameraTestRefreshTacticalDebug()
...
-- BAR metadata is authoritative.  These deliberately small exact-name tables
-- cover known ambiguous roles whose weapon/build flags describe capability,
-- not where players look for the unit in a production wheel.
ControllerCameraTestConstructorClassificationOverrides = ControllerCameraTestConstructorClassificationOverrides or {
	armnanotc = "Build", cornanotc = "Build", legnanotc = "Build",
}
ControllerCameraTestFactoryClassificationOverrides = ControllerCameraTestFactoryClassificationOverrides or {
	armflea = "Utility", armfav = "Utility", corfav = "Utility",
	legscout = "Utility", legscoutveh = "Utility",
}
...
function ControllerCameraTestClassifyBuildOption(unitDef, name)
	...
	local override = ControllerCameraTestConstructorClassificationOverrides[string.lower(tostring(unitDef.name or ""))]
	...
	-- Production infrastructure and construction turrets belong to Build even
	-- when they also carry a defensive weapon.
	if unitDef.isFactory
		or ((unitDef.isBuilder or unitDef.canBuild or unitDef.canAssist)
			and (unitDef.isBuilding or string.find(nameLower, "construction")
				or string.find(nameLower, "nano")))
		or string.find(nameLower, "factory") or string.find(nameLower, "lab")
		or string.find(nameLower, "gantry") then
		return "Build"
	end
...
ControllerCameraTestBuildClassificationCache = ControllerCameraTestBuildClassificationCache or {}

function ControllerCameraTestClassifyFactoryOption(unitDef, name)
	if not unitDef then return "Utility" end
	local nameLower = string.lower(tostring(unitDef.name or name or ""))
	local override = ControllerCameraTestFactoryClassificationOverrides[nameLower]
	if override then return override end
	local mobileBuilder = not unitDef.isBuilding and not unitDef.isFactory
		and (unitDef.isBuilder or unitDef.canBuild
			or (type(unitDef.buildOptions) == "table" and #unitDef.buildOptions > 0))
	if mobileBuilder then return "Constructors" end
	local group = unitDef.customParams and string.lower(tostring(unitDef.customParams.unitgroup or ""))
	local support = unitDef.canTransport
		or (tonumber(unitDef.radarRadius) or 0) > 0
		or (tonumber(unitDef.jammerRadius) or 0) > 0
		or (tonumber(unitDef.sonarRadius) or 0) > 0
		or group == "util" or group == "builder"
		or string.find(nameLower, "scout", 1, true)
		or string.find(nameLower, "transport", 1, true)
		or string.find(nameLower, "radar", 1, true)
		or string.find(nameLower, "jammer", 1, true)
	if support then return "Utility" end
	local armed = unitDef.canAttack == true
		or (type(unitDef.weapons) == "table" and #unitDef.weapons > 0)
	return armed and "Combat" or "Utility"
end
...
function ControllerCameraTestRebuildNativeBuildModel(reason)
	...
	local model = ControllerCameraTestNativeRadialAdapter:BuildBuildModel(items, {
		...
		previousStableKey = menu.selectedStableKey,
		revision = revision,
		classify = function(item, factory)
			local unitDef = item.unitDefID and UnitDefs and UnitDefs[item.unitDefID]
			local cacheKey = tostring(factory and "factory:" or "constructor:")
				.. tostring(item.unitDefID or item.stableKey)
			if not ControllerCameraTestBuildClassificationCache[cacheKey] then
				ControllerCameraTestBuildClassificationCache[cacheKey] = factory
					and ControllerCameraTestClassifyFactoryOption(unitDef, item.name)
					or ControllerCameraTestClassifyBuildOption(unitDef, item.name)
			end
			return ControllerCameraTestBuildClassificationCache[cacheKey]
		end,
	})
	menu.nativeModel = model
	...
	if selected then
		...
		menu.radialPage = selected.globalPage or selected.radialPage or 1
	end
...
function ControllerCameraTestGetBuildSelectionContext(selectedUnits)
	local hasConstructor = false
	for _, unitID in ipairs(type(selectedUnits) == "table" and selectedUnits or {}) do
		if ControllerCameraTestIsSafeSelectableUnit(unitID) then
			local _, unitDef = ControllerCameraTestGetUnitDef(unitID)
			if type(unitDef) == "table" and unitDef.isFactory == true then
				return "factory" -- deterministic mixed-selection priority
			end
			if type(unitDef) == "table" and unitDef.isFactory ~= true
					and ControllerCameraTestIsBuilderUnitDef(unitDef) then
				hasConstructor = true
			end
		end
	end
	return hasConstructor and "constructor" or nil
end

function ControllerCameraTestInvalidateBuildContext(reason)
	local menu = ControllerCameraTestBuildMenu
	if menu.open then ControllerCameraTestCloseBuildMenu(reason or "selection changed") end
	menu.options, menu.radialVisibleOptions, menu.optionCount = {}, {}, 0
	menu.nativeModel, menu.selectedStableKey = nil, nil
	ControllerCameraTestNativeUI.buildModelRevision = -1
	if WG.buildmenu and type(WG.buildmenu.controllerSetFocus) == "function" then
		pcall(WG.buildmenu.controllerSetFocus, nil, "selection-invalidated")
	end
	ControllerCameraTestNativeUI.buildFocus = nil
end
...
function ControllerCameraTestRefreshRadialVisibleOptions()
	...
	if ControllerCameraTestUsesNativeBARUI() and menu.nativeModel then
		local pages = menu.nativeModel.pages or {}
		menu.radialPageCount = math.max(1, #pages)
		menu.radialPage = math.max(1, math.min(menu.radialPageCount, menu.radialPage or 1))
		local page = pages[menu.radialPage]
		for index, option in ipairs(menu.options or {}) do option.menuIndex = index end
		for _, option in ipairs(page and page.entries or {}) do
			menu.radialVisibleOptions[#menu.radialVisibleOptions + 1] = option
		end
		local categoryLabels = {}
		for _, sector in ipairs(page and page.sectors or {}) do categoryLabels[#categoryLabels + 1] = sector.category end
		menu.radialCategoryName = #categoryLabels > 0 and table.concat(categoryLabels, " + ") or filterCat
		menu.radialPageSectors = page and page.sectors or {}
		menu.radialTraversalIndex, menu.radialTraversalCount = menu.radialPage, menu.radialPageCount
	...
	ControllerCameraTestRebuildRadialPageTraversal()
end

-- LB/RB walk one stable sequence across category boundaries.  Category D-pad
-- shortcuts remain available, but paging itself is Economy -> Combat ->
-- Utility -> wrap (and the exact reverse for LB), omitting empty categories.
function ControllerCameraTestRebuildRadialPageTraversal()
	local menu = ControllerCameraTestBuildMenu
	if ControllerCameraTestUsesNativeBARUI() and menu.nativeModel then
		local traversal = {}
		for page = 1, #(menu.nativeModel.pages or {}) do
			traversal[#traversal + 1] = { page = page }
		end
		menu.radialPageTraversal = traversal
		menu.radialTraversalIndex = math.max(1, math.min(#traversal, menu.radialPage or 1))
		menu.radialTraversalCount = math.max(1, #traversal)
		return traversal
	end
	local traversal, ordered = {}, { "Economy", "Combat", "Utility" }
	local function pageCount(category)
		local count, maxPage = 0, 0
		for _, option in ipairs(menu.options or {}) do
			if option.category == category then
				count = count + 1
				maxPage = math.max(maxPage, tonumber(option.radialPage) or 0)
			end
		end
		if count == 0 then return 0 end
		return menu.nativeModel and math.max(1, maxPage) or math.max(1, math.ceil(count / 8))
	end
	for _, category in ipairs(ordered) do
		for page = 1, pageCount(category) do
			traversal[#traversal + 1] = { category = category, page = page }
		end
	end
	-- Unusual native models with none of the standard construction categories
	-- still retain safe paging instead of producing an empty traversal.
	if #traversal == 0 then
		for _, category in ipairs(menu.radialCategories or {}) do
			for page = 1, pageCount(category) do
				traversal[#traversal + 1] = { category = category, page = page }
			end
		end
	...
	menu.radialPageTraversal = traversal
	menu.radialTraversalCount = math.max(1, #traversal)
	menu.radialTraversalIndex = 1
	for index, entry in ipairs(traversal) do
		if entry.category == menu.radialCategoryName and entry.page == menu.radialPage then
			menu.radialTraversalIndex = index
			break
		end
	end
	return traversal
...
function ControllerCameraTestTraverseRadialPages(delta)
	local menu = ControllerCameraTestBuildMenu
	local traversal = ControllerCameraTestRebuildRadialPageTraversal()
	if #traversal <= 1 then
		menu.lastAction = "single global page: no-op"
		return false
	end
	if ControllerCameraTestUsesNativeBARUI() and menu.nativeModel then
		local current = math.max(1, math.min(#traversal, menu.radialPage or 1))
		local index = ((current - 1 + (tonumber(delta) or 1)) % #traversal) + 1
		menu.radialPage = index
		ControllerCameraTestRefreshRadialVisibleOptions()
		local first = menu.radialVisibleOptions[1]
		if first then
			menu.selectedIndex, menu.selectedStableKey = first.menuIndex, first.stableKey
			ControllerCameraTestSetNativeBuildFocus(first, delta < 0 and "global-page-prev" or "global-page-next")
		end
		menu.lastAction = (delta < 0 and "global page previous: " or "global page next: ") .. tostring(index)
		return first ~= nil
	end
	local current
	for index, entry in ipairs(traversal) do
		if entry.category == menu.radialCategoryName and entry.page == menu.radialPage then current = index break end
	end
	if not current then current = delta < 0 and 1 or 0 end
	local index = ((current - 1 + (tonumber(delta) or 1)) % #traversal) + 1
	local entry = traversal[index]
	menu.radialCategoryName, menu.radialPage = entry.category, entry.page
	menu.radialTraversalIndex, menu.radialTraversalCount = index, #traversal
	for categoryIndex, category in ipairs(menu.radialCategories or {}) do
		if category == entry.category then menu.radialCategoryIndex = categoryIndex break end
	end
	ControllerCameraTestRefreshRadialVisibleOptions()
	local first = menu.radialVisibleOptions[1]
	if first then
	...
	menu.lastAction = (delta < 0 and "global page previous: " or "global page next: ")
		.. entry.category .. " " .. tostring(entry.page)
	return first ~= nil
end
...
--------------------------------------------------------------------------------
function ControllerCameraTestFactoryQueueEntryBuildCmdID(cmd)
	if type(cmd) ~= "table" then return nil, 0 end
	local cmdID = tonumber(cmd.id)
	local params = type(cmd.params) == "table" and cmd.params or {}
	if cmdID and cmdID < 0 then
		local quantity = tonumber(params[1]) or tonumber(cmd.count) or 1
		return cmdID, math.max(1, math.floor(quantity))
	end
	if CMD and cmdID == tonumber(CMD.INSERT) then
		local insertedCmdID = tonumber(params[2])
		if insertedCmdID and insertedCmdID < 0 then
			return insertedCmdID, 1
		end
	end
	return nil, 0
end
	...
	for _, unitID in ipairs(selectedUnits) do
		...
		if type(unitDef) == "table" and unitDef.isFactory then
			...
			if type(q) == "table" then
				for i = 1, #q do
					local cmd = q[i]
					local buildCmdID, quantity = ControllerCameraTestFactoryQueueEntryBuildCmdID(cmd)
					if buildCmdID then
						counts[buildCmdID] = (counts[buildCmdID] or 0) + (quantity or 1)
					end
				end
			end
		end
...
function ControllerCameraTestRefreshFactoryQueueModel(reason)
	local menu = ControllerCameraTestBuildMenu
	menu.factoryQueueRefreshReason = tostring(reason or "refresh")
	ControllerCameraTestRefreshFactoryQueueCounts()
	ControllerCameraTestRefreshFactoryQueueProgress()
	if menu.open and ControllerCameraTestUsesNativeBARUI() and menu.isFactoryContext then
		menu.nativeModel = nil
		ControllerCameraTestNativeUI.buildModelRevision = -1
	end
end

function ControllerCameraTestDetectFactoryQuotaMode()
	local menu, api = ControllerCameraTestBuildMenu, WG and WG.ordermenu
	if not (api and type(api.controllerGetCommands) == "function") then
		menu.factoryQuotaModeSource = "ordermenu unavailable"
		return menu.factoryQuotaModeActive == true
	end
	local ok, commands = pcall(api.controllerGetCommands)
	if not ok or type(commands) ~= "table" then
		menu.factoryQuotaModeSource = "ordermenu read failed"
		return menu.factoryQuotaModeActive == true
	end
	for _, descriptor in ipairs(commands) do
		...
		if descriptor.isState == true and (text:find("factoryqueuemode", 1, true)
			...
			local index = tonumber(descriptor.currentStateIndex or params[1]) or 0
			menu.factoryQuotaModeActive = index ~= 0
			menu.factoryQuotaModeSource = "ordermenu state " .. tostring(index)
			return menu.factoryQuotaModeActive
		end
	end
	menu.factoryQuotaModeSource = "descriptor missing"
	return menu.factoryQuotaModeActive == true
end

function ControllerCameraTestGetFactoryQuotaInfo(option)
	if type(option) ~= "table" then return nil end
	local desired = tonumber(option.quotaDesired or option.desiredQuota or option.quotaTarget or option.quota)
	local current = tonumber(option.quotaCount or option.currentQuota or option.quotaCurrent or option.quotaBuilt)
	if (not desired or not current) and type(option.quotaText) == "string" then
		local a, b = string.match(option.quotaText, "^(%d+)%s*/%s*(%d+)$")
		current = current or tonumber(a)
		desired = desired or tonumber(b)
	end
	if (not desired or not current) and WG and WG.Quotas and option.unitDefID then
		local quotas = WG.Quotas
		if not desired and type(quotas.getQuotas) == "function" then
			local ok, quotaTable = pcall(quotas.getQuotas)
			local info = ok and type(quotaTable) == "table" and quotaTable[option.unitDefID] or nil
			if type(info) == "table" then
				desired = tonumber(info.quota or info.desired or info.target or info[1])
				current = current or tonumber(info.current or info.count or info.amount or info[2])
			elseif type(info) == "number" then
				desired = tonumber(info)
			end
		end
		if not current and type(quotas.getUnitAmount) == "function" then
			local builderID = tonumber(option.builderID or option.factoryID)
			local ok, amount = pcall(quotas.getUnitAmount, builderID, option.unitDefID)
			if ok then current = tonumber(amount) end
		end
	end
	if not desired and not current then return nil end
	current = math.max(0, math.floor(tonumber(current) or 0))
	desired = math.max(0, math.floor(tonumber(desired) or 0))
	return { current = current, desired = desired, text = tostring(current) .. "/" .. tostring(desired) }
end
...
local AFFORDABILITY_DISPLAY_SAMPLE_SECONDS = 1.0
local AFFORDABILITY_DISPLAY_ALPHA = 0.35

function ControllerCameraTestBuildAvailabilityKey(option)
	if type(option) ~= "table" then return nil end
	if option.stableKey then return tostring(option.stableKey) end
	if option.unitDefID then return "unit:" .. tostring(option.unitDefID) end
	if option.cmdID then return "cmd:" .. tostring(option.cmdID) end
	return option.name and ("name:" .. tostring(option.name)) or nil
end

function ControllerCameraTestGetBuildResourceDeficits(option)
	...
	return math.max(0, (tonumber(option and option.metalCost) or 0) - currentMetal),
		math.max(0, (tonumber(option and option.energyCost) or 0) - currentEnergy),
		false
end

function ControllerCameraTestRoundBuildDeficitToNearest10(value)
	value = tonumber(value) or 0
	if value < 5 then return 0 end
	return math.floor((value / 10) + 0.5) * 10
end

function ControllerCameraTestFormatBuildAvailabilityParts(metalDeficit, energyDeficit)
	local roundedMetal = ControllerCameraTestRoundBuildDeficitToNearest10(metalDeficit)
	local roundedEnergy = ControllerCameraTestRoundBuildDeficitToNearest10(energyDeficit)
	local parts = {}
	if roundedMetal > 0 then parts[#parts + 1] = "NEED " .. ControllerCameraTestFormatRadialStatNumber(roundedMetal) .. " METAL" end
	if roundedEnergy > 0 then parts[#parts + 1] = "NEED " .. ControllerCameraTestFormatRadialStatNumber(roundedEnergy) .. " ENERGY" end
	return #parts > 0 and table.concat(parts, "  |  ") or nil
end

function ControllerCameraTestGetSmoothedBuildAvailability(option)
	local metalDeficit, energyDeficit, missingApi = ControllerCameraTestGetBuildResourceDeficits(option)
	if missingApi then return nil end
	local key = ControllerCameraTestBuildAvailabilityKey(option)
	if not key then
		return ControllerCameraTestFormatBuildAvailabilityParts(metalDeficit, energyDeficit)
	end
	local menu = ControllerCameraTestBuildMenu
	menu.affordabilityDisplay = type(menu.affordabilityDisplay) == "table" and menu.affordabilityDisplay or {}
	if metalDeficit <= 0 and energyDeficit <= 0 then
		menu.affordabilityDisplay[key] = nil
		return nil
	end
	local now = debugEventTime
	local entry = menu.affordabilityDisplay[key]
	if type(entry) ~= "table" or now >= (entry.nextSampleAt or -math.huge) then
		if type(entry) ~= "table" then
			entry = { metal = metalDeficit, energy = energyDeficit }
		else
			entry.metal = entry.metal + ((metalDeficit - entry.metal) * AFFORDABILITY_DISPLAY_ALPHA)
			entry.energy = entry.energy + ((energyDeficit - entry.energy) * AFFORDABILITY_DISPLAY_ALPHA)
		end
		entry.rawMetal = metalDeficit
		entry.rawEnergy = energyDeficit
		entry.text = ControllerCameraTestFormatBuildAvailabilityParts(entry.metal, entry.energy)
		entry.nextSampleAt = now + AFFORDABILITY_DISPLAY_SAMPLE_SECONDS
		menu.affordabilityDisplay[key] = entry
	end
	return entry.text
end
...
function ControllerCameraTestCanAffordBuildOption(option)
	...
	local metalDeficit, energyDeficit = ControllerCameraTestGetBuildResourceDeficits(option)
	local metalAffordable = metalDeficit <= 0
	local energyAffordable = energyDeficit <= 0
...
function ControllerCameraTestInvalidateAffordabilityCache()
	...
	ControllerCameraTestBuildMenu.affordabilityDisplay = {}
end
...
function ControllerCameraTestOpenBuildMenu()
	...
	if ControllerCameraTestTacticalMenu.open then
		menu.lastAction = "blocked while Tactical Radial is open"
		return false
	end
	local selected = type(spGetSelectedUnits) == "function" and spGetSelectedUnits() or {}
	local context = ControllerCameraTestGetBuildSelectionContext(selected)
	if not context then
		ControllerCameraTestInvalidateBuildContext("no valid build owner")
		menu.lastAction = "no valid build owner"
		return false
	end
	...
	ControllerCameraTestRefreshRadialVisibleOptions()
	ControllerCameraTestRefreshFactoryQueueModel("build radial open")
...
function ControllerCameraTestGetBuildAvailability(option)
	...
	return ControllerCameraTestGetSmoothedBuildAvailability(option)
end
...
function ControllerCameraTestSetPlacementOption(option)
	...
	placement.distributedGridArmed = false
	placement.distributedGridChordOwned = false
	placement.distributedGridResult = "none"
	placement.placementPattern = "single"
...
function ControllerCameraTestCancelPlacement(reason)
	...
	placement.distributedGridArmed = false
	placement.distributedGridChordOwned = false
	placement.distributedGridResult = "cancelled"
...
function ControllerCameraTestHandlePlacementPatternInput()
	local placement = ControllerCameraTestBuildPlacement
	if placement.distributedGridArmed then
		placement.placementPattern = "grid"
		return false
	end
...
function ControllerCameraTestUpdateDistributedGridChord()
	local placement = ControllerCameraTestBuildPlacement
	if not placement.active then
		placement.distributedGridChordOwned = false
		return false
	end
	local owns = ControllerCameraTestActionDown("pitchModifier")
		and ControllerCameraTestActionDown("buildRadial")
		and ControllerCameraTestIsQueueModifierActive()
	placement.distributedGridChordOwned = owns
	if not owns then return false end
	placement.distributedGridArmed = true
	placement.placementPattern = "grid"
	placement.patternPressActive = false
	placement.patternHoldTriggered = false
	placement.lastConstructionShortcut = "LB+RB+RT distributed grid"
	placement.gridShortcutResult = "native distribution armed"
	placement.distributedGridResult = "armed"
	if ControllerInputChords and type(ControllerInputChords.Consume) == "function" then
		ControllerCameraTestInputChord = ControllerInputChords.Consume(
			ControllerCameraTestInputChord, "distributed-grid")
	end
	ControllerCameraTestShowPlacementPatternPopup("grid", "pattern")
	return true
end
...
function ControllerCameraTestDequeueFactoryBuildOption(option)
	...
	if ok then
		...
		ControllerCameraTestRefreshFactoryQueueModel("factory dequeue")
		return true
	else
...
function ControllerCameraTestInsertFactoryBuildOption(option)
	...
	if issuedCount > 0 then
		...
		ControllerCameraTestRefreshFactoryQueueModel("factory insert")
		return true
	end
...
function ControllerCameraTestEnterPlacementFromHighlight()
	...
	menu.isFactoryContext = context == "factory"
	if option.disabled then
...
function ControllerCameraTestHandlePlacementInput(dt)
	...
	local distributedOwns = ControllerCameraTestUpdateDistributedGridChord()

	local drag = ControllerCameraTestDragCommand
	if distributedOwns and drag.active then
		drag.mode = "buildGrid"
		ControllerCameraTestUpdateDragPreview()
	end
	if not distributedOwns and ControllerCameraTestHandleQueueRemovalInput() then
		return true
	end
...
function ControllerCameraTestHandleBuildMenuInput()
		...
		if ControllerCameraTestSelectionPrefersFactoryQueue(selectedUnits) then
			...
			elseif option then
				if ControllerCameraTestUsesNativeBARUI() and WG.buildmenu
					...
					menu.radialLastAction = menu.lastAction
					ControllerCameraTestRefreshFactoryQueueModel("native factory dequeue")
				else
	...
	elseif ControllerCameraTestActionPressed("radialSelect") then
		...
		if ControllerCameraTestSelectionPrefersFactoryQueue(selectedUnits) then
			...
			elseif option and ControllerCameraTestUsesNativeBARUI() and WG.buildmenu
				...
				menu.radialLastAction = menu.lastAction
				ControllerCameraTestRefreshFactoryQueueModel("native factory queue")
			elseif option then
				local queueActive = ControllerCameraTestIsQueueModifierActive()
				...
				if ok then
					...
					menu.radialLastAction = menu.lastAction
					ControllerCameraTestRefreshFactoryQueueModel("factory queue")
				else
	...
	elseif ControllerCameraTestActionPressed("radialPrevPage") then
		ControllerCameraTestTraverseRadialPages(-1)
...
function ControllerCameraTestCancelAreaSelect(reason)
	...
	area.active = false
	area.owner = nil
	area.brushedUnits, area.initialSelection = {}, {}
	area.constructorSet = nil
...
function ControllerCameraTestHandleNormalXInput(dt)
	...
	if ControllerCameraTestActionPressed("smartAction") then
		local generalInsertPress = ControllerCameraTestActionDown("buildRadial")
			and not ControllerCameraTestBuildMenu.open
			and not ControllerCameraTestBuildPlacement.active
			and not ControllerCameraTestDisassemble.active
			and not commandLayerActive
		...
		drag.generalInsertPressActive = generalInsertPress == true
		drag.startX, drag.startY, drag.startZ = reticleWorldX, reticleWorldY, reticleWorldZ
		drag.endX, drag.endY, drag.endZ = reticleWorldX, reticleWorldY, reticleWorldZ
		drag.active = false
		drag.singleUnitPathActive = false
		...
		if drag.generalInsertPressActive then
			ControllerCameraTestGeneralInsert.active = true
			ControllerCameraTestGeneralInsert.lastSource = "Smart X"
			ControllerCameraTestGeneralInsert.lastResult = "Smart X insert armed"
			ControllerCameraTestConsumeRBChord("RB+X general insert")
		end
	end
	...
	if ControllerCameraTestActionReleased("smartAction") and drag.pressActive and drag.pressButton == "smartAction" then
		local generalInsertPress = drag.generalInsertPressActive == true
		if generalInsertPress then
			ControllerCameraTestGeneralInsert.active = true
		end
		if drag.singleUnitPathActive then
			ControllerCameraTestFinishSingleUnitPath()
		...
		if generalInsertPress then
			ControllerCameraTestGeneralInsert.active = false
			ControllerCameraTestGeneralInsert.lastIssued = ControllerCameraTestCommandDebug.issuedCmdID == tostring(CMD and CMD.INSERT)
			ControllerCameraTestGeneralInsert.lastResult = ControllerCameraTestGeneralInsert.lastIssued
				and "Smart X inserted" or "Smart X insert completed"
		end
		drag.pressActive = false
...
function ControllerCameraTestHandleNormalAInput(dt, disassembleMode)
	disassembleMode = disassembleMode == true
	...
	if ControllerCameraTestActionPressed("select") then
		...
		area.owner = disassembleMode and "disassemble" or "normal"
		...
		ControllerCameraTestTraceSingleSelection("DOWN#" .. tostring(area.selectionEdgeID),
			"hovered=" .. tostring(hoveredID) .. " def=" .. tostring(hoveredDefID)
			.. " mode=" .. (disassembleMode and "disassemble" or "normal")
			.. " additive=" .. tostring(area.additive) .. " brush=false mouse=false")
		ControllerCameraTestLayerDebug.areaSelect = area.lastResult
		...
		if area.additive then
			if disassembleMode then
				for unitID, marked in pairs(ControllerCameraTestDisassemble.markedTargets or {}) do
					if marked then area.initialSelection[unitID] = true end
				end
			end
			local sel = type(spGetSelectedUnits) == "function" and spGetSelectedUnits() or {}
			if disassembleMode then sel = ControllerCameraTestNativeDisassembleTargets(sel) end
			for _, uID in ipairs(sel) do
				area.initialSelection[uID] = true
			end
		elseif disassembleMode then
	...
	if area.pressActive and ControllerCameraTestActionDown("select") then
		...
		if area.active then
			...
			if not disassembleMode and ControllerCameraTestActionPressed("smartAction") then
				area.filterRadialOpen = true
				area.highlightedFilter = nil
			end

			if not disassembleMode and area.filterRadialOpen then
			...
			-- Perform live selection brush scanning and touch-accumulation.  Native
			-- Disassemble queries the engine cylinder so hostile units/structures are
			-- represented without trying to put them in Spring's player selection.
			if disassembleMode and reticleHasWorldTarget and reticleWorldX and reticleWorldZ then
				local targets = ControllerCameraTestCollectOwnedTargetsInRadius(
					reticleWorldX, reticleWorldZ, area.radius)
				for _, unitID in ipairs(targets) do area.brushedUnits[unitID] = true end
			elseif reticleHasWorldTarget and reticleWorldX and reticleWorldZ and type(ControllerCameraTestGetVisibleAlliedUnits) == "function" then
				local radiusSq = area.radius * area.radius
				for _, unitID in ipairs(ControllerCameraTestGetVisibleAlliedUnits()) do
					local x, _, z = spGetUnitPosition(unitID)
					if x and z then
						...
						if ((dx * dx) + (dz * dz)) <= radiusSq then
							if not area.brushedUnits[unitID] then
								...
								if unitDef then
									...
									local matchesFilter = disassembleMode
										and ControllerCameraTestIsSafeSelectableUnit(unitID)
										and not (area.constructorSet and area.constructorSet[unitID])
									local filter = area.currentFilter or "All Mobile"
									if disassembleMode then
										-- Disassemble intentionally accepts owned units and structures.
									elseif filter == "All Mobile" then
	...
	if ControllerCameraTestActionReleased("select") and area.pressActive then
		local wasBrush = area.active == true
		if area.active then
			-- Already selected live! Just finalize and clean up brush state.
			area.filterRadialOpen = false
			area.highlightedFilter = nil
			area.lastResult = disassembleMode and "Disassemble live brush selection completed"
				or "live brush selection completed"
			if disassembleMode and not area.additive and (area.lastBrushedCount or 0) == 0 then
				ControllerCameraTestRestoreDisassembleConstructors("Constructors restored")
			end
		elseif disassembleMode then
		...
		ControllerCameraTestTraceSingleSelection("UP#" .. tostring(area.selectionEdgeID or 0),
			"held=" .. string.format("%.3f", debugEventTime - (area.pressStartTime or debugEventTime))
			.. " brush=" .. tostring(wasBrush) .. " mouse=false helper=SelectUnitArray final={"
			.. table.concat(final, ",") .. "}")
		area.pressActive = false
		area.active = false
		area.owner = nil
		area.constructorSet = nil
		area.pressTargetID = nil
		area.pressUnitDefID = nil
	end
	...
	for _, descriptor in ipairs(commands) do
		...
		if descriptor.isState == true and (text:find("factoryqueuemode", 1, true)
			...
			if activated and result == true then
				ControllerCameraTestBuildMenu.factoryQuotaModeActive = nextState ~= 0
				ControllerCameraTestBuildMenu.factoryQuotaModeSource = "toggle state " .. tostring(nextState)
				ControllerCameraTestShowHotkeyFeedback(nextState == 0 and "Queue Mode" or "Quota Mode", "utility")
				return true
			end
			return false
		end
...
function ControllerCameraTestResolveLBLongAction(selected, disassembleActive)
	local context = ControllerCameraTestGetBuildSelectionContext(selected)
	if ControllerCameraTestBuildMenu.open and context == "factory" then
		return "factory-queue-mode"
	end
	if disassembleActive then return "disable-disassemble" end
	if context == "factory" then return nil end
	if ControllerCameraTestSelectionHasDisassembleOperator(selected) then
		return "enable-disassemble"
	end
	return nil
end
...
function ControllerCameraTestUpdateDisassembleController(dt)
	...
	-- Open radials own the shoulders before normal-gameplay arbitration.  This
	-- prevents the RB release from reopening/closing the build radial and keeps
	-- Tactical modal input from leaking into the build shortcut.
	if ControllerCameraTestBuildMenu.open or ControllerCameraTestTacticalMenu.open then
		ControllerCameraTestInputChord = ControllerInputChords.New()
		return false
	end
	local lbDown = ControllerCameraTestActionDown("pitchModifier")
	local rbDown = ControllerCameraTestActionDown("buildRadial")
	local selected = type(spGetSelectedUnits) == "function" and spGetSelectedUnits() or {}
	local longAction = ControllerCameraTestResolveLBLongAction(selected, state.active)
	local event
	event, ControllerCameraTestInputChord = ControllerInputChords.Update(
		...
		ControllerCameraTestSettings.disassembleToggleHoldSeconds, longAction, nil, {
			lbPressed = ControllerCameraTestActionPressed("pitchModifier"),
			lbReleased = ControllerCameraTestActionReleased("pitchModifier"),
			graceSeconds = ControllerCameraTestSettings.rbChordGraceSeconds,
			tapMaxSeconds = ControllerCameraTestSettings.rbTapMaxSeconds,
		})
...
function ControllerCameraTestUpdateLBTapState()
	local cycle = ControllerCameraTestLBCycle
...
function ControllerCameraTestNativeShortcutTarget(targetOverride)
	local target = type(targetOverride) == "table" and targetOverride or ControllerCameraTestGetReticleTargetInfo()
	if target.targetType == "feature" and target.targetID then
		target.commandID = ControllerCameraTestFeatureCommandID(target.targetID)
	end
...
function ControllerCameraTestNativeImmediateShortcut(cmdID, label, targetRequired, explicitParams)
	local api = WG and WG.ordermenu
	if not (api and type(api.controllerIssueCommand) == "function") then return false end
	local accepted, route
	if targetRequired then
		if type(api.controllerExecuteAtTarget) ~= "function" then return false end
		accepted, route = api.controllerExecuteAtTarget(cmdID, ControllerCameraTestNativeShortcutTarget(),
			ControllerCameraTestGetCommandOptions(), ControllerCameraTestIsQueueFrontModifierActive() and "insert-front" or nil)
	else
		accepted, route = api.controllerIssueCommand(cmdID, explicitParams or {},
			ControllerCameraTestGetCommandOptions(), ControllerCameraTestIsQueueFrontModifierActive() and "insert-front" or nil)
	end
	if accepted then
		ControllerCameraTestShowHotkeyFeedback(string.upper(label), "utility")
		ControllerCameraTestLayerDebug.commandLayerAction = label .. " via native " .. tostring(route)
	end
	return accepted == true
end

function ControllerCameraTestNativeRepeatShortcut()
	local api = WG and WG.ordermenu
	if not (api and type(api.controllerGetCommands) == "function"
			and type(api.controllerActivateState) == "function") then return false end
	local commands = api.controllerGetCommands()
	for _, descriptor in ipairs(type(commands) == "table" and commands or {}) do
		if tonumber(descriptor.cmdID) == tonumber((CMD and CMD.REPEAT) or 115) then
			local nextState = (tonumber(descriptor.currentStateIndex) or 0) == 0 and 1 or 0
			local accepted = api.controllerActivateState(descriptor.cmdID, nextState)
			if accepted then ControllerCameraTestShowHotkeyFeedback("REPEAT", "utility") end
			return accepted == true
		end
	end
	return false
...
function widget:ActiveCommandChanged(cmdID, cmdType)
	ControllerCameraTestNativeTargeting.descriptorDirty = true
	if ControllerCameraTestNativeTargeting.anchor then
		ControllerCameraTestTraceNativeTarget("active-command-changed",
			"cached operation retained; new=" .. tostring(cmdID))
	else
		ControllerCameraTestRefreshNativeTargetingDescriptor(true)
	end
end
...
function ControllerCameraTestPointInRect(x, y, rect)
...
function ControllerCameraTestTryPregamePrimaryAction(mx, my)
	if Spring.GetGameFrame() > 0 then
		return false
	end
...
function ControllerCameraTestTryLuaUIClick(mx, my)
...
function ControllerCameraTestTryRmlClick(mx, my)
...
function ControllerCameraTestTryPregamePlacement(mx, my)
	if Spring.GetGameFrame() > 0 then
		return false
	end
...
function ControllerCameraTestUpdateControllerModeAndCommandLayer(dt)
	...
	local rbxGeneralInsertBusy = not stagedTacticalBusy and not distributedPlacementOwns
		and not placementBusy and not rtRbSuppressed
		and ControllerCameraTestUpdateRBXGeneralInsert()
	local rbTypeNavigationBusy = not stagedTacticalBusy and not distributedPlacementOwns
		and not placementBusy and not rtRbSuppressed and not rbxGeneralInsertBusy
		and ControllerCameraTestUpdateRBMobileTypeNavigation()
	local nativeTargetingBusy = not stagedTacticalBusy and not distributedPlacementOwns
		and not placementBusy and not rtRbSuppressed and not rbxGeneralInsertBusy and not rbTypeNavigationBusy
	...
	local disassembleBusy = not stagedTacticalBusy and not distributedPlacementOwns
		and not placementBusy and not rtRbSuppressed and not rbxGeneralInsertBusy
		and not rbTypeNavigationBusy and not nativeTargetingBusy and not priorityTacticalToggle
	...
	if not stagedTacticalBusy and not distributedPlacementOwns and not placementBusy
			and not rtRbSuppressed and not rbxGeneralInsertBusy and not rbTypeNavigationBusy and not nativeTargetingBusy
	...
	elseif rbxGeneralInsertBusy then
		commandLayerActive = false
		if ControllerCameraTestAreaSelect.pressActive or ControllerCameraTestAreaSelect.active then
			ControllerCameraTestCancelAreaSelect("cancelled by RB+X insert")
		end
	elseif rbTypeNavigationBusy then
		commandLayerActive = false
		if ControllerCameraTestAreaSelect.pressActive or ControllerCameraTestAreaSelect.active then
			ControllerCameraTestCancelAreaSelect("cancelled by RB+D-pad type navigation")
		end
	...
	elseif disassembleBusy then
		commandLayerActive = false
		if ControllerCameraTestAreaSelect.owner ~= "disassemble"
				and (ControllerCameraTestAreaSelect.pressActive or ControllerCameraTestAreaSelect.active) then
...
function ControllerCameraTestUpdateNativeInputMode()
	...
	local hideNative = state.controllerStable == true and ControllerCameraTestSettings.nativePanelsDebugVisible ~= true
	local panelVisible = not hideNative
	for _, api in ipairs({ (WG and WG.buildmenu) or false, (WG and WG.ordermenu) or false }) do
		if api and type(api.controllerSetPanelVisible) == "function" then pcall(api.controllerSetPanelVisible, panelVisible) end
		if api and type(api.controllerSetFocusVisible) == "function" then
			pcall(api.controllerSetFocusVisible, panelVisible and ControllerCameraTestSettings.showNativeFocusStroke == true)
		end
	end
	if WG.buildmenu and type(WG.buildmenu.controllerSetInputActive) == "function" then
		pcall(WG.buildmenu.controllerSetInputActive, state.controllerStable == true)
	end
end
...
function ControllerCameraTestBuildCompactSelectedStatus()
	...
	if info.mode == "factory" then
		if status.refreshUnitID ~= info.unitID or debugEventTime >= (status.nextRefreshTime or 0) then
			ControllerCameraTestRefreshFactoryQueueModel("compact factory status")
...
function ControllerCameraTestDrawBuildRadial()
	...
	if Spring.GetGameFrame() % 15 == 0 then
		ControllerCameraTestRefreshFactoryQueueModel("factory radial draw")
	end
	...
	local quotaMode = isFactoryContext and ControllerCameraTestDetectFactoryQuotaMode()
	local quotaCenterFill = { 0.30, 0.035, 0.24, 0.92 }
	local quotaAccent = { 1.0, 0.38, 0.82, 1.0 }
	...
	local selectedOption = ControllerCameraTestGetRadialCurrentOption()
	local catKey = string.lower((selectedOption and selectedOption.category) or menu.radialCategoryName or "economy")
	...
	if ControllerUISharedRenderers then
		...
		for index, option in ipairs(visibleOptions) do
			...
			local quotaInfo = quotaMode and ControllerCameraTestGetFactoryQuotaInfo(option) or nil
			local parsedQueueCount = isFactoryContext and option.cmdID and menu.factoryQueueCounts
				and menu.factoryQueueCounts[option.cmdID] or nil
			local queueCount = quotaMode and 0 or parsedQueueCount
				or (ControllerCameraTestUsesNativeBARUI() and (option.queueCount or 0) or 0)
			local affordable = ControllerCameraTestCanAffordBuildOption(option)
			entries[index] = { label = option.shortLabel or option.name or "Build", texture = option.iconTexture,
				...
				progress = progress, badge = queueCount, quotaText = quotaInfo and quotaInfo.text or nil,
				quotaCount = quotaInfo and quotaInfo.current or nil, quotaDesired = quotaInfo and quotaInfo.desired or nil,
				indexLabel = option.radialSlot or index,
				slot = option.radialSlot or index, nativeCell = option, queueCount = queueCount,
		...
		local sectors = {}
		for _, sector in ipairs(menu.radialPageSectors or {}) do
			local colors = BuildRadialPageColors[string.lower(sector.category or "")] or BuildRadialPageColors.utility
			sectors[#sectors + 1] = {
				category = sector.category, label = sector.category,
				firstSlot = sector.firstSlot, lastSlot = sector.lastSlot, count = sector.count,
				fill = colors.fill, accent = colors.accent, labelColor = colors.label or colors.accent,
			}
		end
		ControllerUISharedRenderers.DrawRadial({
			bounds = { x1 = cx - radius * 1.45, y1 = cy - radius * 1.45, x2 = cx + radius * 1.45, y2 = cy + radius * 1.45 },
			model = { style = "build", title = info and info.title or (current and (current.name or current.shortLabel)) or (isFactoryContext and "Factory" or "Build"), selectedTitle = isFactoryContext,
				...
				categoryLabel = menu.radialCategoryName or (isFactoryContext and "Factory" or "Build"), sectors = sectors,
				pageLabel = "PAGE " .. tostring(menu.radialTraversalIndex or 1) .. "/" .. tostring(menu.radialTraversalCount or 1),
				heading = quotaMode and "QUOTA MODE" or nil,
				centerFill = quotaMode and quotaCenterFill or nil,
				centerAccent = quotaMode and quotaAccent or nil,
			...
			entryRenderer = ControllerCameraTestUsesNativeBARUI() and ControllerNativeBuildCellRenderer
				and function(entry, rect, isSelected)
					local option = entry.nativeCell or {}
					return ControllerNativeBuildCellRenderer.Draw({
						rect = rect, flow = WG.FlowUI, unitDefID = option.unitDefID,
						texture = option.iconTexture, metalCost = option.metalCost,
						energyCost = option.energyCost, queueCount = entry.queueCount,
						progress = entry.progress, quotaText = entry.quotaText,
						disabled = option.disabled, unaffordable = entry.unaffordable,
						underConstruction = option.underConstruction,
						zoom = isSelected and 0.2025 or 0.0375,
						radarTexture = option.radarTexture, groupTexture = option.groupTexture,
					})
				end or nil,
		})
		return
	...
	for i = 1, n do
		...
		-- Draw Factory Queue/Quota badge if needed
		if isFactoryContext and option.cmdID and menu.factoryQueueCounts then
			local quotaInfo = quotaMode and ControllerCameraTestGetFactoryQuotaInfo(option) or nil
			local qCount = quotaMode and 0 or (menu.factoryQueueCounts[option.cmdID] or 0)
			local badgeText = quotaInfo and quotaInfo.text or (qCount > 0 and ("x" .. tostring(qCount)) or nil)
			if badgeText then
				...
				local badgeAccent = quotaMode and quotaAccent or pageColor.accent
				gl.Color(badgeAccent[1], badgeAccent[2], badgeAccent[3], 0.7)
				gl.LineWidth(1)
				gl.BeginEnd(GL.LINE_LOOP, function()
					gl.Vertex(bx1, by1)
					gl.Vertex(bx2, by1)
				...
				if quotaMode then gl.Color(1.0, 0.50, 0.88, 1)
				else gl.Color(1, 0.95, 0.8, 1) end
	...
	if currentOption then
		...
		if quotaMode then gl.Color(quotaCenterFill[1], quotaCenterFill[2], quotaCenterFill[3], quotaCenterFill[4])
		else gl.Color(0.015, 0.04, 0.065, 0.88) end
		drawCircle(cx, cy, panelRadius, 40)
		local centerAccent = quotaMode and quotaAccent or pageColor.accent
		gl.Color(centerAccent[1], centerAccent[2], centerAccent[3], 0.78)
		gl.LineWidth(1.5)
		gl.BeginEnd(GL.LINE_LOOP, function()
			for i = 0, 40 do
		...
		if quotaMode then
			gl.Color(1.0, 0.45, 0.86, 0.98)
			gl.Text("QUOTA MODE", cx, cy + panelRadius * 0.84, math.max(9, labelSize * 1.15), "oc")
		end
		...
		local affordable = ControllerCameraTestCanAffordBuildOption(currentOption)
	...
	local pageStr = "PAGE " .. tostring(menu.radialTraversalIndex or 1) .. "/" .. tostring(menu.radialTraversalCount or 1)
...
function ControllerCameraTestDrawHelpOverlay()
	...
	local lines = {
		...
		"Selection: A selects exactly one friendly hovered ID | RT+A toggles that exact ID | A hold area-selects units first",
...
function ControllerCameraTestDrawNativeTargetDiagnosticStrip()
	if not ControllerCameraTestSettings.debugPanelVisible then return end
	local api = WG and WG.ordermenu
	if not (api and type(api.controllerGetTargetDiagnostics) == "function") then return end
	local ok, diagnostics = pcall(api.controllerGetTargetDiagnostics)
	if not ok or type(diagnostics) ~= "table" then return end
	local events = diagnostics.events or {}
	local first = math.max(1, #events - 3)
	local rows = {}
	for i = first, #events do rows[#rows + 1] = tostring(events[i]) end
	if #rows == 0 then
		rows[1] = string.format("SELF-CHECK owners=%s areaMex=%s reclaim=%s dispatcher=%s",
			tostring(diagnostics.ownerCount or 0), tostring(diagnostics.areaMexRegistered == true),
			tostring(diagnostics.smartAreaReclaimRegistered == true),
			tostring(diagnostics.dispatcherConnected == true))
	end
	local width, rowHeight = math.min(viewSizeX - 40, 720), 18
	local x1, y1 = (viewSizeX - width) * 0.5, viewSizeY - 44 - (#rows * rowHeight)
	gl.Color(0.01, 0.02, 0.03, 0.90)
	gl.Rect(x1, y1, x1 + width, y1 + #rows * rowHeight + 10)
	gl.Color(0.55, 1.0, 0.72, 1)
	for index, row in ipairs(rows) do
		gl.Text(row, x1 + 8, y1 + (#rows - index) * rowHeight + 8, 13, "o")
	end
	gl.Color(1, 1, 1, 1)
end
...
function widget:DrawScreen()
	...
	ControllerCameraTestDrawNativeTargetDiagnosticStrip()
	...
	if ControllerCameraTestDebugCompact then
		local compactLines = {
			...
			"Idle: " .. tostring(ControllerCameraTestIdleCycle.lastTypeName) .. " x" .. tostring(ControllerCameraTestIdleCycle.lastCount) .. " | Group " .. ControllerCameraTestGetControlGroupDisplaySlot(activeGroupSlot) .. " " .. tostring(activeGroupType) .. " x" .. tostring(activeGroupCount) .. " | A: " .. tostring(lastSelectionResult),
...
function widget:GetConfigData()
	...
	return {
		...
		nativeGroupMigrationComplete = ControllerCameraTestControlGroups.nativeMigrationComplete == true,
		nativeGroupMigrationStatus = ControllerCameraTestControlGroups.nativeMigrationStatus,
	}
end
...
function widget:SetConfigData(data)
	...
	ControllerCameraTestControlGroups.nativeMigrationComplete = data.nativeGroupMigrationComplete == true
	ControllerCameraTestControlGroups.nativeMigrationStatus = data.nativeGroupMigrationStatus or "not required"
	if not ControllerCameraTestControlGroups.nativeMigrationComplete and type(data.controllerGroups) == "table"
			and type(Spring.GetGroupUnits) == "function" and type(Spring.SetUnitGroup) == "function" then
		local migrated = 0
		for rawSlot, entry in pairs(data.controllerGroups) do
			local slot = ControllerCameraTestNormalizeControlGroupSlot(rawSlot)
			local groupNumber = ControllerCameraTestNativeGroupNumber(slot)
			if #(Spring.GetGroupUnits(groupNumber) or {}) == 0 and type(entry) == "table" then
				for _, unitID in ipairs(ControllerCameraTestFilterValidUnits(entry.units or {})) do
					Spring.SetUnitGroup(unitID, groupNumber)
					migrated = migrated + 1
				end
			end
		end
		ControllerCameraTestControlGroups.nativeMigrationComplete = true
		ControllerCameraTestControlGroups.nativeMigrationStatus = "migrated " .. tostring(migrated) .. " legacy memberships into empty native groups"
	end
	if not ControllerCameraTestControlGroups.nativeMigrationComplete then
		ControllerCameraTestControlGroups.nativeMigrationComplete = true
		ControllerCameraTestControlGroups.nativeMigrationStatus = "no legacy controller memberships found"
	end
	ControllerCameraTestApplySettingsDefaults()
	ControllerCameraTestSettings.debugPanelVisible = false
	ControllerCameraTestEnsureBindings()
...
function widget:SelectionChanged(selectedUnits)
	...
	ControllerCameraTestInvalidateBuildContext("selection changed")
end
...
function widget:DrawWorld()
	...
	if ControllerCameraTestAreaSelect.active and reticleHasWorldTarget then
		...
		local disassembleBrush = ControllerCameraTestAreaSelect.owner == "disassemble"
		-- Stroke
		gl.LineWidth(2.5)
		gl.Color(disassembleBrush and 0.2 or 0.25, disassembleBrush and 1.0 or 0.75,
			disassembleBrush and 0.46 or 1.0, 0.75)
		gl.DrawGroundCircle(reticleWorldX, reticleWorldY, reticleWorldZ, R, 48)

		-- Fill (concentric rings for smooth look, 22% opacity)
		gl.LineWidth(2.0)
		gl.Color(disassembleBrush and 0.2 or 0.25, disassembleBrush and 1.0 or 0.75,
			disassembleBrush and 0.46 or 1.0, 0.22)
		local step = R / 25
