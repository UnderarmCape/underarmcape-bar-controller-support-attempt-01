...
Metadata and capability checks are authoritative. Two small exact-name override tables cover known ambiguity only: `armnanotc`, `cornanotc`, and `legnanotc` remain Build; `armflea`, `corak`, `armfav`, `corfav`, `legscout`, and `legscoutveh` remain Utility. Classification is cached by context plus UnitDef ID and is deterministic.
