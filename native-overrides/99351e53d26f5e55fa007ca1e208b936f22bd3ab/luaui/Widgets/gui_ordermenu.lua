...
function widget:Initialize()
	...
	-- Export the already-filtered command panel model.  Callers receive shallow
	-- copies so controller focus cannot mutate BAR's authoritative descriptors.
	WG['ordermenu'].controllerGetCommands = function()
		local result = {}
		for i = 1, #commands do
			local cmd = commands[i]
			result[i] = {
	...
	WG['ordermenu'].controllerActivate = function(cmdID, direction)
		...
		if not cmd or cmd.disabled then
			return false
		end
		local button = (direction or 1) < 0 and 3 or 1
		if cmd.id == CMD.FIRE_STATE then
			local virtualIndex = cmd.virtualIndex or 1
			local desired = OrderMenuFirestate.nextCycledVirtualIndex(virtualIndex, button ~= 1)
			OrderMenuFirestate.giveVirtualIndex(desired, 0)
			doUpdate = true
			return true
		end
		local cmdDescIndex = Spring.GetCmdDescIndex(cmd.id)
