...
function widget:Initialize()
	...
	-- Keep controller selection in the engine-owned selection model.  Direct
	-- reticle clicks intentionally use vanilla click eligibility; area input may
	-- opt into the same SmartSelect exclusions used by the mouse box path.
	local function controllerSelectable(unitID, applySmartFilter)
		if not unitID or spGetUnitNoSelect(unitID) then
			return false
		end
		local unitDefID = spGetUnitDefID(unitID)
		if not unitDefID or ignoreUnits[unitDefID] or spGetUnitTeam(unitID) ~= myTeamID then
			return false
		end
		return not applySmartFilter or smartSelectIncludeFilter(unitDefID)
	end

	WG['smartselect'].controllerApplyUnits = function(units, mode, applySmartFilter)
		local current = spGetSelectedUnits()
		local result, present = {}, {}
		if mode ~= 'replace' then
			for i = 1, #current do
				local unitID = current[i]
				present[unitID] = true
		...
		for i = 1, #(units or {}) do
			...
			if controllerSelectable(unitID, applySmartFilter == true) then
				if mode == 'toggle' and present[unitID] then
					present[unitID] = nil
					for j = #result, 1, -1 do
						if result[j] == unitID then
							table.remove(result, j)
							break
						end
					end
				elseif not present[unitID] then
					present[unitID] = true
					result[#result + 1] = unitID
				end
			end
		end
		selectedUnits = result
		spSelectUnitArray(result)
		return result
	end

	WG['smartselect'].controllerSelectUnit = function(unitID, mode)
		return WG['smartselect'].controllerApplyUnits({ unitID }, mode or 'replace', false)
	end
	WG['smartselect'].controllerIsSelectable = controllerSelectable
	WG['smartselect'].controllerApiVersion = 1
