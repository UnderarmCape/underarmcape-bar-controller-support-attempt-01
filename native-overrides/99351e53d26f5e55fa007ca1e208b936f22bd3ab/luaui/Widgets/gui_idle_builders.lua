...
local controllerCursorUnitID
local controllerCursorTypeID
local controllerRevision = 0
local controllerLastResult = "idle"
...
local function controllerSnapshot()
	local units, types = {}, {}
	for _, unitDefID in ipairs(existingIcons) do
		local copy = {}
		for _, unitID in ipairs(idleList[unitDefID] or {}) do
			if spValidUnitID(unitID) and not spGetUnitIsDead(unitID) then
				copy[#copy + 1], units[#units + 1] = unitID, unitID
			end
		end
		if #copy > 0 then types[#types + 1] = { unitDefID = unitDefID, units = copy } end
	end
	return {
		units = units, types = types,
		currentUnitID = controllerCursorUnitID,
		currentTypeID = controllerCursorTypeID,
		revision = controllerRevision,
		lastResult = controllerLastResult,
	}
end
