...
local controllerFormationOwner
local controllerOwnerCommands = {}
...
function widget:DrawWorld()
    ...
    local ownerState = controllerFormationOwner and controllerFormationOwner:GetState() or nil
    local ownerPreview = ownerState and ownerState.anchor and ownerState.current
    if #fNodes > 1 or #dimmNodes > 1 or ownerPreview then
		...
		if ownerPreview then
			local savedNodes, savedDists, savedLength, savedCmd = fNodes, fDists, lineLength, usingCmd
			local dx, dz = ownerState.current.x - ownerState.anchor.x, ownerState.current.z - ownerState.anchor.z
			local distance = sqrt(dx * dx + dz * dz)
			fNodes = {
				{ ownerState.anchor.x, ownerState.anchor.y, ownerState.anchor.z },
				{ ownerState.current.x, ownerState.current.y, ownerState.current.z },
			}
			fDists, lineLength, usingCmd = { 0, distance }, distance, ownerState.cmdID
			if distance > 0 then DrawFormationDots(tVerts, zoomY) end
			fNodes, fDists, lineLength, usingCmd = savedNodes, savedDists, savedLength, savedCmd
		end
		glColor(1,1,1,1)
    end
end
...
function widget:Update(deltaTime)
    ...
    if dimmAlpha <= 0 then
        ...
        if #fNodes == 0 and not (controllerFormationOwner and controllerFormationOwner:IsActive()) then
...
function widget:Initialize()
	...
	WG.customformations.EndFormation = function(worldPos, cmdID, controllerOptions)
		if #fNodes == 0 then return false end
		...
		if usingFormation then
			...
			local alt, ctrl, meta, shift = GetModKeys()
			if type(controllerOptions) == "table" then
				local function enabled(name)
					if controllerOptions[name] ~= nil then return controllerOptions[name] == true end
					for i = 1, #controllerOptions do if controllerOptions[i] == name then return true end end
					return false
				end
	...
	controllerFormationOwner = ControllerNativeCommandOwner.New({
		name = "Custom Formations", types = CMDTYPE,
		dispatch = function(cmdID, params, options, dispatchMode)
			...
			local notifyOptions = { alt = false, ctrl = false, meta = false, shift = false, right = false }
			if type(options) == "table" then
				for key, value in pairs(options) do
					local name = type(key) == "number" and tostring(value) or tostring(key)
					if notifyOptions[name] ~= nil and (type(key) == "number" or value == true) then notifyOptions[name] = true end
				end
			end
			local ok, handled = pcall(widgetHandler.CommandNotify, widgetHandler, cmdID, params, notifyOptions)
			if not ok then return false, "CommandNotify failed" end
			if handled then return true, "widget" end
			local began = WG.customformations.StartFormation({ params[1], params[2], params[3] }, cmdID, false)
			if not began then return false, "formation start rejected" end
			pathCandidate = false
			WG.customformations.AddFormationNode({ params[4], params[5], params[6] })
			local issued = WG.customformations.EndFormation(nil, cmdID, notifyOptions)
		...
		onCancel = function()
			if WG.customformations then WG.customformations.CancelFormation() end
		end,
	})
	WG.ControllerNativeCommandOwners = WG.ControllerNativeCommandOwners or {}
	for cmdID in pairs(formationCmds) do
		controllerOwnerCommands[cmdID] = true
		WG.ControllerNativeCommandOwners[cmdID] = { api = controllerFormationOwner, name = "Custom Formations" }
		local api = WG and WG.ordermenu
		if api and type(api.controllerRegisterCommandOwner) == "function" then
			api.controllerRegisterCommandOwner(cmdID, controllerFormationOwner, "Custom Formations")
		end
	end
end
...
function widget:Shutdown()
	local api = WG and WG.ordermenu
	for cmdID in pairs(controllerOwnerCommands) do
		if api and type(api.controllerUnregisterCommandOwner) == "function" then
			api.controllerUnregisterCommandOwner(cmdID, controllerFormationOwner)
		end
