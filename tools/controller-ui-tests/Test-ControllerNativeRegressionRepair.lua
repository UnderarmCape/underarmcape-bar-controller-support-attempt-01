...
test(38, "Ordinary idle navigation no longer needs vanilla snapshot API", function()
	local start = assert(camera:find("function ControllerCameraTestGetIdleCycleUnits", 1, true))
	local finish = assert(camera:find("function ControllerCameraTestUnitTypeName", start, true))
	local source = camera:sub(start, finish)
	return has(source, "ControllerCameraTestGetOwnTeamUnits()")
		and not has(source, "controllerGetLiveIdleEntries")
