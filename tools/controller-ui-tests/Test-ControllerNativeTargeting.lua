...
Targeting.SetDescriptor(state, { id = 10, type = CMDTYPE.ICON_MAP, name = "Move" }, CMDTYPE)
...
Targeting.SetDescriptor(state, { id = 20, type = CMDTYPE.ICON_UNIT, name = "Guard" }, CMDTYPE)
...
expect(params == nil and kind == "allied unit required", "guard rejects enemy unit")

Targeting.SetDescriptor(state, { id = 25, type = CMDTYPE.ICON_UNIT_OR_MAP, name = "Attack" }, CMDTYPE)
...
Targeting.SetDescriptor(state, { id = 90, type = CMDTYPE.ICON_UNIT_FEATURE_OR_AREA, name = "Reclaim" }, CMDTYPE)
params, kind = Targeting.BeginOrBuildPoint(state,
	{ targetType = "feature", targetID = 9, commandID = 32009 }, CMDTYPE)
expect(kind == "feature" and params[1] == 32009, "feature command uses engine-version-aware encoded ID")
params, kind = Targeting.BeginOrBuildPoint(state, { targetType = "unit", targetID = 12 }, CMDTYPE)
expect(kind == "unit" and params[1] == 12, "unit-feature-area accepts direct unit")
...
expect(state.phase == Targeting.CONTROLLER_AREA_TARGETING and state.shape == "area", "area latch state is explicit")
...
Targeting.SetDescriptor(state, { id = 16, type = CMDTYPE.ICON_FRONT, name = "Fight" }, CMDTYPE)
params, kind = Targeting.BeginOrBuildPoint(state, { targetType = "ground", x = 0, y = 1, z = 0 }, CMDTYPE)
expect(kind == "anchor" and state.phase == Targeting.FRONT_TARGETING, "front command latches first point")
...
Targeting.SetDescriptor(state, { id = 30, type = CMDTYPE.ICON_UNIT_OR_RECTANGLE, name = "Select rectangle" }, CMDTYPE)
params, kind = Targeting.BeginOrBuildPoint(state, { targetType = "unit", targetID = 77 }, CMDTYPE)
expect(kind == "unit" and params[1] == 77, "unit-or-rectangle preserves direct unit")
...
Targeting.SetDescriptor(state, { id = -101, type = CMDTYPE.ICON_BUILDING, name = "Solar" }, CMDTYPE)
expect(state.phase == Targeting.BUILD_PLACEMENT, "negative building descriptor is delegated to placement")
Targeting.SetDescriptor(state, { id = 5, type = CMDTYPE.ICON_MODE, name = "Fire State" }, CMDTYPE)
...
expect(contains(camera, "drag.nativeControllerTargeting"), "controller anchor has one custom preview owner")
...
expect(contains(orders, "Spring.GiveOrder, expectedCmdID"), "unhandled command reaches engine exactly once")
...
expect(contains(targetHandler, "BeginOrBuildPoint"), "first press resolves direct target or anchor")
expect(contains(targetHandler, "BuildAnchoredParams"), "second press confirms anchored geometry")
expect(contains(targetHandler, "UpdatePreview"), "cursor movement updates latched geometry")
expect(contains(camera, "controllerIssueActiveTarget"), "camera emits one logical target dispatch")
expect(contains(camera, "persistentUntilQueueRelease"), "queued persistence has explicit ownership")
expect(contains(camera, 'ControllerCameraTestCancelActiveCommandTargeting("queue modifier released"'), "RT release ends repeated targeting")
...
expect(contains(orders, "if not source or source.disabled then return nil end"), "disabled/stale active descriptor rejected")
...
expect(contains(checklist, "24. Watch for duplicate orders"), "24-step live checklist covers duplicate/performance watch")
