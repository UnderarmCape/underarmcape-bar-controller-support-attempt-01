...
test(1, "Central version is 0.8.0 Experimental", function() return has(props, ">0.8.0<") and has(props, ">Experimental<") end)
test(2, "Bridge banner matches exactly", function() return has(metadata, 'BridgeBanner => "BAR Controller Bridge " + DisplayVersion') and has(read("tools/controller-companion/Tests/Program.cs"), '"BAR Controller Bridge v0.8.0 Experimental"') end)
...
test(4, "Installer/package metadata matches", function() return has(installer, "ProductMetadata.DisplayVersion") and has(packageManifest, '"version": "0.8.0"') and has(packageScript, "ControllerCompanionSemanticVersion") end)
...
test(14, "Same-type units are not added", function() return lacks(camera, "ControllerCameraTestSelectVisibleSameTypeUnderReticle") and lacks(camera, "ControllerCameraTestSelectAllOwnedSameTypeUnderReticle") end)
...
test(16, "Rapid single taps remain deterministic", function() return lacks(normalA, "lastTapTime) <=") and lacks(normalA, "doubleTap") end)
...
test(21, "Targeting A suppresses normal selection", function() local p=camera:find("local nativeTargetBusy",1,true); return p < camera:find("ControllerCameraTestHandleNormalAInput(dt",p,true) end)
test(22, "Build placement A suppresses normal selection", function() local p=camera:find("local nativeTargetBusy",1,true); return camera:find("ControllerCameraTestHandlePlacementInput(dt)",p,true) < camera:find("ControllerCameraTestHandleNormalAInput(dt",p,true) end)
...
test(27, "Tactical selection does not anchor", function() return has(tacticalExecution, '"tactical-radial", false') and lacks(tacticalExecution, "BeginOrBuildPoint") end)
...
test(32, "Smart X cannot steal placement X", function() return has(camera, "nativeTargetBusy") and has(camera, "ControllerCameraTestActionPressed(\"smartAction\")") end)
test(33, "Normal selection cannot steal placement A", function() return has(camera, "nativeTargetBusy") and has(camera, "ControllerCameraTestActionPressed(\"select\")") end)
...
test(44, "Native transformation receives final shape", function() return has(mex,"controllerCompleteArea") and has(reclaim,"controllerCompleteArea") and has(formations,"controllerCompleteShape") end)
...
test(108, "Dead or non-idle stored ID is repaired", function() return has(camera,"if Spring.GetUnitDefID(unitID) == ControllerCameraTestIdleCycle.currentTypeKey") end)
