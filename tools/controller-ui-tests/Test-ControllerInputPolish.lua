...
test(7, "Camera movement does not translate anchor", function() return has(camera, "state.anchor.x, state.anchor.y, state.anchor.z") end)
...
test(14, "Exactly one dispatch occurs", function() return has(camera, "ControllerCameraTestIssueNativeTarget(params)") and has(order, "if handled then return true") end)
...
test(17, "Cached command works if GetActiveCommand changes or clears", function() return has(order, "cachedDescriptor") and has(camera, "engine active command change ignored") end)
test(18, "B cancels", function() return has(camera, 'ControllerCameraTestCancelActiveCommandTargeting("targeting cancelled by B"') end)
...
test(38, "RB release rearms the next tap", function() return has(chordsSource, "RB owns rearming") and has(chordsSource, "if not rbDown then") end)
...
test(117, "Native same-type area reclaim is preferred where valid", function() return has(camera, "ControllerCameraTestConfirmNativeReclaim()") and has(smartReclaim, "controllerTargeting.mode == 'same'") end)
...
test(139, "Radial compaction tests pass", function() return has(adapter, "categoryItems") and has(adapter, "pageCounts") end)
...
test(143, "Move State sub-radial passes", function() return has(camera, 'kind = "move_state_cycle"') end)
