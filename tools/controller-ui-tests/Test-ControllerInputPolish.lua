...
test(19, "No normal A selection leaks through", function() return has(camera, "local stagedTacticalBusy = ControllerCameraTestHandleStagedTacticalCommandInput()") and has(camera, "if stagedTacticalBusy then") end)
...
test(117, "Restored same-type area reclaim is preferred where valid", function() return has(camera, "ControllerCameraTestStartNativeSameTypeReclaim") and has(camera, "area.waitingForNeutral") end)
