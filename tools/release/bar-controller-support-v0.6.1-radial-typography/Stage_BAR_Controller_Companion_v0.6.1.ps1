...
$targetName = 'BAR_Controller_Support_v0.6.1_Widget_Companion'
...
$releaseBodyPath = Join-Path $packageRootFull 'BAR_Controller_Support_v0.6.1.release.md'
...
$runtimeProjects = [ordered]@{
    ...
    installer = [ordered]@{ project = 'tools\controller-companion\Installer\BARControllerCompanionInstaller.csproj'; file = 'BAR_Controller_Companion_Installer_v0.6.1.exe' }
    restore = [ordered]@{ project = 'tools\controller-companion\Restore\BARControllerCompanionRestore.csproj'; file = 'BAR_Controller_Companion_Restore_v0.6.1.exe' }
}
...
function Build-FrozenRuntimeArtifacts {
    ...
    try {
        ...
        Invoke-SmokeProcess (Join-Path $runtimeArtifactsRootFull 'installer\BAR_Controller_Companion_Installer_v0.6.1.exe') @('--release-smoke-test') 2 'Installer'
        Invoke-SmokeProcess (Join-Path $runtimeArtifactsRootFull 'restore\BAR_Controller_Companion_Restore_v0.6.1.exe') @('--release-smoke-test') 2 'Restore'
...
$copies = [ordered]@{
    $outputs.installer = (Join-Path $target 'BAR_Controller_Companion_Installer_v0.6.1.exe')
    $outputs.restore = (Join-Path $target 'BAR_Controller_Companion_Restore_v0.6.1.exe')
    ...
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.6.1\README.md') = (Join-Path $target 'docs\README.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.6.1\RELEASE_NOTES_v0.6.1.md') = (Join-Path $target 'docs\RELEASE_NOTES_v0.6.1.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.6.1\PACKAGE_INVENTORY_v0.6.1.md') = (Join-Path $target 'docs\PACKAGE_INVENTORY_v0.6.1.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.6.1\FROZEN_RUNTIME_MANIFEST.md') = (Join-Path $target 'docs\FROZEN_RUNTIME_MANIFEST.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.6.1\RADIAL_TYPOGRAPHY_ARCHITECTURE.md') = (Join-Path $target 'docs\RADIAL_TYPOGRAPHY_ARCHITECTURE.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.6.1\HSV_COLOR_EDITOR.md') = (Join-Path $target 'docs\HSV_COLOR_EDITOR.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.6.1\MIGRATION_AND_KNOWN_LIMITATIONS.md') = (Join-Path $target 'docs\MIGRATION_AND_KNOWN_LIMITATIONS.md')
}
...
$releaseTemplate = [IO.File]::ReadAllText((Join-Path $PSScriptRoot 'controller-support-v0.6.1-radial-typography.release.md'))
