...
if ([string]::IsNullOrWhiteSpace($RuntimeArtifactsRoot)) {
    $RuntimeArtifactsRoot = Join-Path $WorkspaceRoot 'artifacts\v0.7.0\win-x64'
}

$releaseVersion = '0.7.0'
$fileVersion = '0.7.0.0'
$runtimeIdentifier = 'win-x64'
$targetName = 'BAR_Controller_Support_v0.7.0_Widget_Companion'
...
$releaseBodyPath = Join-Path $packageRootFull 'BAR_Controller_Support_v0.7.0.release.md'
...
$runtimeProjects = [ordered]@{
    ...
    installer = [ordered]@{ project = 'tools\controller-companion\Installer\BARControllerCompanionInstaller.csproj'; file = 'BAR_Controller_Companion_Installer_v0.7.0.exe' }
    restore = [ordered]@{ project = 'tools\controller-companion\Restore\BARControllerCompanionRestore.csproj'; file = 'BAR_Controller_Companion_Restore_v0.7.0.exe' }
}
...
function Build-FrozenRuntimeArtifacts {
    ...
    try {
        ...
        Invoke-SmokeProcess (Join-Path $runtimeArtifactsRootFull 'installer\BAR_Controller_Companion_Installer_v0.7.0.exe') @('--release-smoke-test') 2 'Installer'
        Invoke-SmokeProcess (Join-Path $runtimeArtifactsRootFull 'restore\BAR_Controller_Companion_Restore_v0.7.0.exe') @('--release-smoke-test') 2 'Restore'
...
foreach ($directory in @(
    ...
    (Join-Path $target 'luaui\Images\controller\stat-icons'),
    (Join-Path $target 'controller-ui'),
    (Join-Path $target 'docs')
)) { New-Item -ItemType Directory -Force -Path $directory | Out-Null }

$copies = [ordered]@{
    $outputs.installer = (Join-Path $target 'BAR_Controller_Companion_Installer_v0.7.0.exe')
    $outputs.restore = (Join-Path $target 'BAR_Controller_Companion_Restore_v0.7.0.exe')
    ...
    (Join-Path $workspaceRootFull 'luaui\Images\controller\stat-icons\metal.png') = (Join-Path $target 'luaui\Images\controller\stat-icons\metal.png')
    (Join-Path $workspaceRootFull 'luaui\Images\controller\stat-icons\energy.png') = (Join-Path $target 'luaui\Images\controller\stat-icons\energy.png')
    (Join-Path $workspaceRootFull 'luaui\Images\controller\stat-icons\health.png') = (Join-Path $target 'luaui\Images\controller\stat-icons\health.png')
    (Join-Path $workspaceRootFull 'luaui\Images\controller\stat-icons\asset-manifest.json') = (Join-Path $target 'luaui\Images\controller\stat-icons\asset-manifest.json')
    (Join-Path $workspaceRootFull 'luaui\Images\controller\stat-icons\LICENSE.md') = (Join-Path $target 'luaui\Images\controller\stat-icons\LICENSE.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.7.0\README.md') = (Join-Path $target 'docs\README.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.7.0\RELEASE_NOTES_v0.7.0.md') = (Join-Path $target 'docs\RELEASE_NOTES_v0.7.0.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.7.0\PACKAGE_INVENTORY_v0.7.0.md') = (Join-Path $target 'docs\PACKAGE_INVENTORY_v0.7.0.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.7.0\FROZEN_RUNTIME_MANIFEST.md') = (Join-Path $target 'docs\FROZEN_RUNTIME_MANIFEST.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.7.0\RADIAL_TYPOGRAPHY_ARCHITECTURE.md') = (Join-Path $target 'docs\RADIAL_TYPOGRAPHY_ARCHITECTURE.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.7.0\HSV_COLOR_EDITOR.md') = (Join-Path $target 'docs\HSV_COLOR_EDITOR.md')
    (Join-Path $workspaceRootFull 'doc\controller-companion-v0.7.0\MIGRATION_AND_KNOWN_LIMITATIONS.md') = (Join-Path $target 'docs\MIGRATION_AND_KNOWN_LIMITATIONS.md')
}
foreach ($name in @('controller_socket_bridge.lua', 'gui_controller_camera_test.lua', 'gui_pregameui.lua', 'cmd_area_mex.lua',
    'gui_controller_bindings_ui.lua', 'gui_controller_smartx_mouse_audit.lua', 'gui_controller_ui_runtime.lua', 'gui_controller_ui_layout.lua')) {
    $copies[(Join-Path $workspaceRootFull ('luaui\Widgets\' + $name))] = Join-Path $target ('luaui\Widgets\' + $name)
}
foreach ($name in @('controller_disassemble_behavior.lua', 'controller_ui_runtime.lua', 'controller_ui_editor_workspace.lua', 'controller_ui_editor_input.lua', 'controller_ui_shared_renderers.lua', 'controller_glyphs.lua')) {
...
try {
    $fixedTime = [DateTimeOffset]::new(2026, 7, 21, 0, 0, 0, [TimeSpan]::Zero)
...
$releaseTemplate = [IO.File]::ReadAllText((Join-Path $PSScriptRoot 'controller-support-v0.7.0-disassemble-mode.release.md'))
