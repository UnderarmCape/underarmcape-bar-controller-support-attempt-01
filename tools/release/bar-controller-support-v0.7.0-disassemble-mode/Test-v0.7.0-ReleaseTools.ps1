...
$packageRoot = Join-Path $PackageArtifactsRoot 'BAR_Controller_Support_v0.7.0_Widget_Companion'
$zipPath = Join-Path $PackageArtifactsRoot 'BAR_Controller_Support_v0.7.0_Widget_Companion.zip'
...
if ([string]::IsNullOrWhiteSpace($PublicV061PackageRoot)) {
    $PublicV061PackageRoot = Join-Path $PackageArtifactsRoot 'BAR_Controller_Support_v0.6.1_Widget_Companion'
}
...
$installer = Join-Path $packageRoot 'BAR_Controller_Companion_Installer_v0.7.0.exe'
$restore = Join-Path $packageRoot 'BAR_Controller_Companion_Restore_v0.7.0.exe'
$runtimeManifestPath = Join-Path $packageRoot 'frozen-runtime-manifest.json'
$testRoot = [IO.Path]::GetFullPath((Join-Path $WorkspaceRoot 'test-output\v0.7.0-release-isolated'))
...
return {
    ...
    data = {
        ["Controller UI Layout"] = {
            personal = "keep",
            personalSettings = {
                elements = { bindings = { x = 0.90958327, y = 0.82051295 } },
                favoriteColors = { "#336699" },
            },
            authorData = { selectedCategory = "Hints" },
        },
        unrelated = { value = 17 },
    },
    order = {
...
function Assert-Installed([object]$Environment) {
    ...
    foreach ($name in @('Controller Camera Test', 'Controller Bindings UI', 'Controller UI Runtime')) {
        if ($configText -notmatch ('(?m)^\s*\["' + [regex]::Escape($name) + '"\]\s*=\s*[1-9]\d*\s*,')) {
            throw "Required widget is not enabled: $name"
        }
    }
    if ($configText -notmatch '(?m)^\s*\["Controller UI Layout"\]\s*=\s*0\s*,') {
        throw 'Optional Controller UI Layout authoring widget is not disabled.'
    }
    if ($configText -notmatch '(?m)^\s*\["Controller UI Runtime"\]\s*=\s*\{' -or
        ([regex]::Matches($configText, 'personal\s*=\s*"keep"')).Count -ne 2 -or
        ([regex]::Matches($configText, 'x\s*=\s*0\.90958327')).Count -ne 2 -or
        ([regex]::Matches($configText, 'y\s*=\s*0\.82051295')).Count -ne 2) {
        throw 'Controller UI Layout data was not copied exactly into the production runtime data table.'
    }
...
if (-not (Test-Path -LiteralPath $installer -PathType Leaf) -or -not (Test-Path -LiteralPath $restore -PathType Leaf)) {
    throw 'v0.7.0 package must be staged before running release tests.'
}
...
if ($LASTEXITCODE -ne 0 -or [string]$runtimeManifest.sourceCommit -ne $sourceCommit -or [string]$runtimeManifest.version -ne '0.7.0') {
...
foreach ($exeName in @('BAR_Controller_Companion_Installer_v0.7.0.exe', 'BAR_Controller_Companion_Restore_v0.7.0.exe',
    ...
    if ($version -ne '0.7.0.0') { throw "$exeName has unexpected file version $version" }
}
...
Assert-Installed $clean
$secondConfig = [IO.File]::ReadAllText($clean.Config)
if (([regex]::Matches($secondConfig, '(?m)^\s*\["Controller UI Runtime"\]\s*=\s*\{')).Count -ne 1) {
    throw 'Second install duplicated Controller UI Runtime data.'
}
...
function Test-LegacyUpgrade([string]$LegacyVersion, [string]$LegacyRoot) {
    ...
    Invoke-Exe $installer $upgradeArguments "v$LegacyVersion to v0.7.0 upgrade"
    ...
    if ([string]$upgradeState.Version -ne '0.7.0') { throw "v$LegacyVersion upgrade state version is not 0.7.0." }
}

Test-LegacyUpgrade '0.6.1' $PublicV061PackageRoot
Test-LegacyUpgrade '0.6.0' $PublicV060PackageRoot
Test-LegacyUpgrade '0.5.1' $LegacyPackageRoot

Write-Host 'PASS: v0.7.0 clean install, public v0.6.1/v0.6.0/v0.5.1 upgrades, layout-to-runtime migration, idempotency, restore, payload hashes, executable versions, and package exclusions'
