...
$packageRoot = Join-Path $PackageArtifactsRoot 'BAR_Controller_Support_v0.6.0_Widget_Companion'
$zipPath = Join-Path $PackageArtifactsRoot 'BAR_Controller_Support_v0.6.0_Widget_Companion.zip'
...
foreach ($exeName in @('BAR_Controller_Companion_Installer_v0.6.0.exe', 'BAR_Controller_Companion_Restore_v0.6.0.exe',
    ...
    if ($version -ne '0.6.0.0') { throw "$exeName has unexpected file version $version" }
}
...
if (-not (Test-Path -LiteralPath $LegacyPackageRoot -PathType Container)) { throw "Legacy v0.5.1 package missing: $LegacyPackageRoot" }
$legacyInstaller = Join-Path $LegacyPackageRoot 'BAR_Controller_Companion_Installer_v0.5.1.exe'
$upgrade = New-FakeEnvironment 'upgrade-v0.5.1'
$legacyArguments = @('--package-root', $LegacyPackageRoot, '--bar-data', $upgrade.BarData, '--install-root', $upgrade.Install, '--shortcut', $upgrade.Shortcut, '--no-pause')
Invoke-Exe $legacyInstaller $legacyArguments 'Legacy v0.5.1 installer'
...
[IO.File]::WriteAllText($upgradePersonal, '{"bindings":"v0.5.1-custom","positions":{"hints":[0.2,0.3]},"profiles":["mine"]}', (New-Object Text.UTF8Encoding($false)))
...
Invoke-Exe $installer $upgradeArguments 'v0.5.1 to v0.6.0 upgrade'
Assert-Installed $upgrade
if ((Get-Sha256 $upgradePersonal) -ne $upgradeHash) { throw 'Upgrade changed personal controller state.' }
$upgradeState = Get-Content -Raw -LiteralPath (Join-Path $upgrade.Install 'install-state.json') | ConvertFrom-Json
if ([string]$upgradeState.Version -ne '0.6.0') { throw 'Upgrade state version is not 0.6.0.' }

Write-Host 'PASS: v0.6.0 clean install, real v0.5.1 upgrade, idempotency, restore, payload hashes, executable versions, and package exclusions'
