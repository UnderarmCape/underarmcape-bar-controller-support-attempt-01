...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) { $OutputRoot = Join-Path $RepositoryRoot 'artifacts\v0.8.5-public-release' }
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $RepositoryRoot 'doc\controller-companion-v0.8.5') -Destination (Join-Path $stageRoot 'doc\controller-companion-v0.8.5') -Recurse
    ...
    Copy-Payload (Join-Path $RepositoryRoot 'tools\release\bar-controller-support-v0.8.5\README.md') (Join-Path $stageRoot 'README.md')
    Copy-Payload (Join-Path $RepositoryRoot 'tools\release\bar-controller-support-v0.8.5\RELEASE_NOTES_v0.8.5.md') (Join-Path $stageRoot 'RELEASE_NOTES_v0.8.5.md')
    ...
    $manifest = [ordered]@{
        ...
        releaseSummary = 'RB+X general front insertion, RB+A factory front insertion, factory queue/quota presentation, RB+D-pad mobile type navigation, stricter idle/disassemble eligibility, compact same-type reclaim radius, contextual hints, automatic updates, Recovery Mode, and transactional self-update.'
...
& (Join-Path $PSScriptRoot 'BAR_Controller_Companion_Installer_v0.8.5_Experimental.exe') --package-root $PSScriptRoot --bar-data $BarDataPath --install-root $InstallRoot --no-pause
exit $LASTEXITCODE
'@
    [IO.File]::WriteAllText((Join-Path $stageRoot 'Install_v0.8.5.ps1'), $installScript, (New-Object Text.UTF8Encoding($false)))
    ...
    [IO.File]::Copy((Join-Path $stageRoot 'RELEASE_NOTES_v0.8.5.md'), (Join-Path $OutputRoot 'RELEASE_NOTES_v0.8.5.md'), $true)
