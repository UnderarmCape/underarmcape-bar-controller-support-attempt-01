...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) { $OutputRoot = Join-Path $RepositoryRoot 'artifacts\v0.8.4-public-release' }
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $RepositoryRoot 'doc\controller-companion-v0.8.4') -Destination (Join-Path $stageRoot 'doc\controller-companion-v0.8.4') -Recurse
    ...
    Copy-Payload (Join-Path $RepositoryRoot 'tools\release\bar-controller-support-v0.8.4\README.md') (Join-Path $stageRoot 'README.md')
    Copy-Payload (Join-Path $RepositoryRoot 'tools\release\bar-controller-support-v0.8.4\RELEASE_NOTES_v0.8.4.md') (Join-Path $stageRoot 'RELEASE_NOTES_v0.8.4.md')
    ...
    $manifest = [ordered]@{
        ...
        releaseSummary = 'RB+A/RB+X general command insertion, RT+RB hard suppression, Smart X repair/reclaim priority, expanded idle pools, split Disassemble friendly/enemy target ownership, direct custom Self Destruct, automatic updates, Recovery Mode, and transactional self-update.'
...
& (Join-Path $PSScriptRoot 'BAR_Controller_Companion_Installer_v0.8.4_Experimental.exe') --package-root $PSScriptRoot --bar-data $BarDataPath --install-root $InstallRoot --no-pause
exit $LASTEXITCODE
'@
    [IO.File]::WriteAllText((Join-Path $stageRoot 'Install_v0.8.4.ps1'), $installScript, (New-Object Text.UTF8Encoding($false)))
    ...
    [IO.File]::Copy((Join-Path $stageRoot 'RELEASE_NOTES_v0.8.4.md'), (Join-Path $OutputRoot 'RELEASE_NOTES_v0.8.4.md'), $true)
