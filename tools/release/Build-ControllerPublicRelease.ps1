...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) { $OutputRoot = Join-Path $RepositoryRoot 'artifacts\v0.8.3-public-release' }
...
try {
    ...
    Copy-Item -LiteralPath (Join-Path $RepositoryRoot 'doc\controller-companion-v0.8.3') -Destination (Join-Path $stageRoot 'doc\controller-companion-v0.8.3') -Recurse
    ...
    Copy-Payload (Join-Path $RepositoryRoot 'tools\release\bar-controller-support-v0.8.3\README.md') (Join-Path $stageRoot 'README.md')
    Copy-Payload (Join-Path $RepositoryRoot 'tools\release\bar-controller-support-v0.8.3\RELEASE_NOTES_v0.8.3.md') (Join-Path $stageRoot 'RELEASE_NOTES_v0.8.3.md')
    ...
    $manifest = [ordered]@{
        ...
        releaseSummary = 'Smart X direct repair, unified RB+A Insert Queue, Disassemble chord priority, same-target double-tap selection, Tactical state/Wait persistence, protected Self Destruct, automatic updates, Recovery Mode, and transactional self-update.'
...
& (Join-Path $PSScriptRoot 'BAR_Controller_Companion_Installer_v0.8.3_Experimental.exe') --package-root $PSScriptRoot --bar-data $BarDataPath --install-root $InstallRoot --no-pause
exit $LASTEXITCODE
'@
    [IO.File]::WriteAllText((Join-Path $stageRoot 'Install_v0.8.3.ps1'), $installScript, (New-Object Text.UTF8Encoding($false)))
    ...
    [IO.File]::Copy((Join-Path $stageRoot 'RELEASE_NOTES_v0.8.3.md'), (Join-Path $OutputRoot 'RELEASE_NOTES_v0.8.3.md'), $true)
