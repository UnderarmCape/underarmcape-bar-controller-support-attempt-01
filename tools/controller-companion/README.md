# BAR Controller Companion v0.8.1 Experimental development

The experimental source version is centralized in `Directory.Build.props`. All projects consume semantic version `0.8.1`, channel `Experimental`, and display version `v0.8.1 Experimental`; `ProductMetadata` derives runtime banners and artifact names. Public releases now use the mandatory manifest-driven GitHub workflow. Every successful future milestone must invoke `tools/release/Publish-ControllerRelease.ps1`, include schema-v1 sidecars, and become GitHub Latest unless it is historical Recovery Mode data. Deployment alone is not completion.
