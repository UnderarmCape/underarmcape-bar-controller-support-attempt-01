...
$RequiredBranch = 'controller/v0.6.0-ui-qol-overhaul'
$RequiredWidgets = @(
...
$projects = @(
    ...
    'tools\controller-ui-publisher\BARControllerUIDefaultsPublisher.csproj'
)
$runtimeProjects = @(
    'tools\controller-companion\BarControllerCompanion.csproj',
    'tools\controller-companion\Launcher\BARControllerLauncher.csproj',
...
if (-not $SkipBuild) {
    Write-Step 'Building Release projects.'
    foreach ($project in $projects) {
        Invoke-NativeChecked -FilePath 'dotnet' -Arguments @('build', (Join-Path $RepositoryRoot $project), '-c', 'Release') -Label ('Build ' + $project)
    }
    foreach ($project in $runtimeProjects) {
        Invoke-NativeChecked -FilePath 'dotnet' -Arguments @('publish', (Join-Path $RepositoryRoot $project), '-c', 'Release', '-r', 'win-x64', '--self-contained', 'true') -Label ('Publish ' + $project)
    }
}
...
$publishedBridge = Join-Path $RepositoryRoot 'tools\controller-companion\bin\Release\net5.0\win-x64\publish\BARControllerBridge.exe'
$publishedLauncher = Join-Path $RepositoryRoot 'tools\controller-companion\Launcher\bin\Release\net5.0\win-x64\publish\BARControllerLauncher.exe'
$publishedRestore = Join-Path $RepositoryRoot 'tools\controller-companion\Restore\bin\Release\net5.0\win-x64\publish\BAR_Controller_Companion_Restore_v0.6.0.exe'
...
$backupRoot = Join-Path $CompanionInstallPath ('deployment-backups\v0.6.0-' + $timestamp)
...
Write-Output ('Rollback: powershell -NoProfile -ExecutionPolicy Bypass -File "' + (Join-Path $RepositoryRoot 'tools\dev-scripts\Rollback_v0.6.0_Live_BAR.ps1') + '" -BackupRoot "' + $backupRoot + '"')
