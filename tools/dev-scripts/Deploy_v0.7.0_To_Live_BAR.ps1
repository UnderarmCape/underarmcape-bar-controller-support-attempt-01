...
if ([string]::IsNullOrWhiteSpace($RuntimeArtifactsRoot)) {
    $RuntimeArtifactsRoot = Join-Path $RepositoryRoot 'artifacts\v0.7.0\win-x64'
}

$RequiredBranch = 'controller/v0.7.0-disassemble-mode'
$RequiredWidgets = @(
    'Controller Camera Test',
    'Controller Bindings UI',
    'Controller UI Runtime'
)
$DisabledWidgets = @('Controller UI Layout')
$WidgetFiles = @(
...
$StatAssetFiles = @('metal.png', 'energy.png', 'health.png', 'asset-manifest.json', 'LICENSE.md')
...
function Enable-RequiredWidgets([string]$ConfigPath, [string]$ReplacementBackupPath) {
    ...
    if ($content -notmatch '(?m)^[ \t]*\["Controller UI Runtime"\][ \t]*=[ \t]*\{') {
        $dataMatch = [regex]::Match($content, '(?m)^[ \t]*data[ \t]*=[ \t]*\{')
        if (-not $dataMatch.Success) { throw 'BYAR.lua does not contain a data table.' }
        $dataOpen = $content.IndexOf('{', $dataMatch.Index)
        $dataClose = Find-MatchingBrace -Content $content -OpeningBrace $dataOpen
        $dataBody = $content.Substring($dataOpen + 1, $dataClose - $dataOpen - 1)
        $layoutMatch = [regex]::Match($dataBody, '(?m)^(?<indent>[ \t]*)\["Controller UI Layout"\][ \t]*=[ \t]*\{')
        if ($layoutMatch.Success) {
            $layoutStart = $dataOpen + 1 + $layoutMatch.Index
            $layoutOpen = $content.IndexOf('{', $layoutStart)
            $layoutClose = Find-MatchingBrace -Content $content -OpeningBrace $layoutOpen
            $layoutValue = $content.Substring($layoutOpen, $layoutClose - $layoutOpen + 1)
            $dataNewline = if ($content.Contains("`r`n")) { "`r`n" } else { "`n" }
            $insertion = $dataNewline + $layoutMatch.Groups['indent'].Value + '["Controller UI Runtime"] = ' + $layoutValue + ','
            $content = $content.Substring(0, $dataClose) + $insertion + $content.Substring($dataClose)
        }
    }
    ...
    $widgetOrders = @($RequiredWidgets | ForEach-Object { [pscustomobject]@{ Name = $_; Order = 1 } }) +
        @($DisabledWidgets | ForEach-Object { [pscustomobject]@{ Name = $_; Order = 0 } })
    foreach ($widget in $widgetOrders) {
        $widgetName = [string]$widget.Name
        $widgetOrder = [int]$widget.Order
        $pattern = '(?m)^(?<indent>[ \t]*)\["' + [regex]::Escape($widgetName) + '"\][ \t]*=[ \t]*(?<value>-?\d+)[ \t]*,?[ \t]*(?=\r?$)'
        $match = [regex]::Match($body, $pattern)
        if ($match.Success) {
            if ([int]$match.Groups['value'].Value -ne $widgetOrder) {
                $replacement = $match.Groups['indent'].Value + '["' + $widgetName + '"] = ' + $widgetOrder + ','
                $body = $body.Substring(0, $match.Index) + $replacement + $body.Substring($match.Index + $match.Length)
            }
        }
        else {
            $entry = $indent + '["' + $widgetName + '"] = ' + $widgetOrder + ','
...
foreach ($widgetFile in $WidgetFiles) {
    ...
    $widgetUpvalues = Get-MaxLuaUpvalues -Path $source
    if ($widgetUpvalues -gt 60) { throw "$widgetFile captures $widgetUpvalues upvalues; BAR permits 60." }
}
...
$StatAssetDirectory = Join-Path $BarDataPath 'LuaUI\Images\controller\stat-icons'
...
foreach ($assetFile in $StatAssetFiles) {
    $source = Join-Path $RepositoryRoot ('luaui\images\controller\stat-icons\' + $assetFile)
    if (-not (Test-Path -LiteralPath $source -PathType Leaf)) { throw "Controller stat asset is missing: $source" }
}
...
Invoke-NativeChecked -FilePath 'lua' -Arguments @((Join-Path $RepositoryRoot 'tools\controller-ui-tests\Test-ControllerUIReleaseQuality.lua'), $RepositoryRoot) -Label 'Controller UI v0.7.0 release-quality harness'
...
Invoke-NativeChecked -FilePath 'lua' -Arguments @((Join-Path $RepositoryRoot 'tools\controller-ui-tests\Test-ControllerDisassembleMode.lua'), $RepositoryRoot) -Label 'Controller disassemble behavior harness'
Invoke-NativeChecked -FilePath 'lua' -Arguments @((Join-Path $RepositoryRoot 'tools\controller-ui-tests\Test-ControllerUIRuntime.lua'), $RepositoryRoot) -Label 'Controller lean runtime harness'
...
if ([string]$runtimeManifest.version -ne '0.7.0' -or [string]$runtimeManifest.sourceCommit -ne $commit) {
    throw 'Frozen runtime manifest does not match v0.7.0 and the checked-out source commit.'
}
...
$publishedRestore = Join-Path $RuntimeArtifactsRoot 'restore\BAR_Controller_Companion_Restore_v0.7.0.exe'
...
$backupRoot = Join-Path $CompanionInstallPath ('deployment-backups\v0.7.0-' + $timestamp)
...
foreach ($assetFile in $StatAssetFiles) {
    Backup-LiveFile (Join-Path $StatAssetDirectory $assetFile) (Join-Path 'live-before\LuaUI\Images\controller\stat-icons' $assetFile) 'controller stat icon asset'
}
...
$deployMap = @(
    ...
    [pscustomobject]@{ source = (Join-Path $RepositoryRoot 'luaui\Widgets\gui_controller_ui_runtime.lua'); destination = (Join-Path $WidgetDirectory 'gui_controller_ui_runtime.lua') },
    ...
    [pscustomobject]@{ source = (Join-Path $RepositoryRoot 'luaui\Include\controller_disassemble_behavior.lua'); destination = (Join-Path $LuaIncludeDirectory 'controller_disassemble_behavior.lua') },
    [pscustomobject]@{ source = (Join-Path $RepositoryRoot 'luaui\Include\controller_ui_runtime.lua'); destination = (Join-Path $LuaIncludeDirectory 'controller_ui_runtime.lua') },
    ...
    [pscustomobject]@{ source = (Join-Path $RepositoryRoot 'luaui\images\controller\stat-icons\metal.png'); destination = (Join-Path $StatAssetDirectory 'metal.png') },
    [pscustomobject]@{ source = (Join-Path $RepositoryRoot 'luaui\images\controller\stat-icons\energy.png'); destination = (Join-Path $StatAssetDirectory 'energy.png') },
    [pscustomobject]@{ source = (Join-Path $RepositoryRoot 'luaui\images\controller\stat-icons\health.png'); destination = (Join-Path $StatAssetDirectory 'health.png') },
    [pscustomobject]@{ source = (Join-Path $RepositoryRoot 'luaui\images\controller\stat-icons\asset-manifest.json'); destination = (Join-Path $StatAssetDirectory 'asset-manifest.json') },
    [pscustomobject]@{ source = (Join-Path $RepositoryRoot 'luaui\images\controller\stat-icons\LICENSE.md'); destination = (Join-Path $StatAssetDirectory 'LICENSE.md') },
    ...
    [pscustomobject]@{ source = $publishedRestore; destination = (Join-Path $CompanionInstallPath 'BAR_Controller_Companion_Restore_v0.7.0.exe') },
...
$installedStatManifest = Get-Content -Raw -Encoding UTF8 (Join-Path $StatAssetDirectory 'asset-manifest.json') | ConvertFrom-Json
if ($installedStatManifest.kind -ne 'bar-controller-stat-icons' -or @($installedStatManifest.assets).Count -ne 3) {
    throw 'Installed stat icon manifest is invalid or incomplete.'
}
foreach ($asset in $installedStatManifest.assets) {
    $installedAsset = Join-Path $StatAssetDirectory ([string]$asset.file)
    if ((Get-Sha256 $installedAsset) -ne [string]$asset.sha256) { throw "Installed stat icon hash mismatch: $($asset.file)" }
}
...
$layoutOrder = [regex]::Match($configText, '\["Controller UI Layout"\]\s*=\s*(?<order>-?\d+)')
if (-not $layoutOrder.Success -or [int]$layoutOrder.Groups['order'].Value -ne 0) { throw 'Controller UI Layout authoring widget is not disabled.' }
if ($configText -notmatch '(?m)^[ \t]*\["Controller UI Runtime"\][ \t]*=[ \t]*\{') { throw 'Controller UI runtime data was not migrated.' }
...
Write-JsonAtomic -Path $manifestPath -Value $manifest
Invoke-NativeChecked -FilePath 'powershell' -Arguments @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File',
    (Join-Path $RepositoryRoot 'tools\dev-scripts\Rollback_v0.7.0_Live_BAR.ps1'), '-BackupRoot', $backupRoot, '-ValidateOnly') -Label 'Rollback validation'

Write-Step "Deployment succeeded for $branch at $commit."
Write-Output ('Backup root: ' + $backupRoot)
Write-Output ('Rollback: powershell -NoProfile -ExecutionPolicy Bypass -File "' + (Join-Path $RepositoryRoot 'tools\dev-scripts\Rollback_v0.7.0_Live_BAR.ps1') + '" -BackupRoot "' + $backupRoot + '"')
